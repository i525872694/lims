...
while($row = $DB->fetch_assoc($rec_query)) {
	...
	if(empty($row['water_type'])){
		if(!empty($row['kb_sid'])){
                $yuanyang_sites = $DB->fetch_one_assoc("SELECT `water_type` FROM `sites` WHERE `id`='{$row['kb_sid']}'");
		...
		$lx=array_filter(explode(',',$slx['water_type']));
		if (is_array($lx) && count($lx)>=1) {
			$water_type_maxs = [];
			foreach ($lx as $value) {
				$water_type_maxs[]=get_water_type_max($value,$fzx_id);
			}
			$water_type_maxs=implode(',',$water_type_maxs);
            $water_type_maxs=ltrim($water_type_maxs,",");
			$water_type_str='AND x.lxid in ('.$water_type_maxs.')';
		}
		else
		{
			$water_type_str='AND x.lxid ='.$water_type;
		}
	}else{
