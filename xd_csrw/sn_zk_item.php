...
if($cy_rs['status']>='6'){
	...
	$save_button="<center><input type=\"submit\" value=\"保存\"></center>";
}
if(!empty($current_value_C))
{
	...
	foreach($current_value_C as $k =>$v)
	{
		...
		if($current_nums==$i&&$add_tds!=5){
			$current_line.="<td width='20%'><span><input type=\"checkbox\" checked=\"checked\" name=\"vid[]\" value=\"{$k}\" />".$v."</span></td>";
			for($j=0;$j<$add_tds;$j++){
				$current_line.="<td width='20%'>&nbsp;</td>";
			}
		}
		else
		$current_line.="<td><span><input type=\"checkbox\" checked=\"checked\" name=\"vid[]\" value=\"{$k}\" />".$v."</span></td>";
...
if(!empty($opt_value_C)){
	foreach($opt_value_C as $k2=>$v2){
		...
		if($no_select_nums==$i&&$add_tds!=5){
			$no_select_line.="<td width='20%'><span><input type=\"checkbox\" name=\"vid[]\" value=\"{$k2}\" $disabled />".$v2."</span></td>";
				for($j=0;$j<$add_tds;$j++){
