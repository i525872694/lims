...
elseif(!empty($_POST['csrw_xdcs_user'])){
	...
	//签字后同时生成化验单
	gotourl("$rooturl/xd_csrw/create_hyd.php?cyd_id={$cyd_id}&&cyd_bh={$cyd_bh}");
}else{
	$cy_sql="UPDATE `cy` SET  jcwc_date='{$_POST['jcwc_date']}',cy_dept='{$_POST['cy_dept']}',jc_dept='{$_POST['jc_dept']}',jc_yiju='{$_POST['jc_yiju']}',
	...
	//获取批次的信息 并更新到对接表中
	//获取批次信息
	$cy_data = curl_request($rooturl."/cy/cy_tzd.php?cyd_id={$cyd_id}&action=update");
	//同步到对接数据库
	$cy_data = json_decode($cy_data,true);
	$curl_data = array();
	$curl_data['rec_arr'] = $cy_data['rec_arr'];
	$curl_data['fzx_id'] = $fzx_id;
	$data = curl_request($duijie_url."xd_cyrw/cy_duijie_url.php?ajax=1",$curl_data);
}
if(empty($_POST['hid'])){
	gotourl("fp_csrw.php?cyd_id={$cyd_id}&&cyd_bh={$cyd_bh}");
}
?>
