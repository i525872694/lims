		...
		if(!empty($xcpx_item_arrs)){
			foreach($xcpx_item_arrs as $key=>$value){
				...
				if(!empty($xcpx_del_vids)){
					$xcpx_rs['site_name']=$xcpx_rs['site_name'];
...
function remove_vid_after_create_hyd($cy_rec, $vid_ary) {
	...
	//循环每个要删除的项目
		foreach($vid_ary as $key=>$value) {
			...
			$res=$DB->fetch_one_assoc("SELECT * FROM assay_order WHERE tid = '".$tid."'");
			if(!$res) {
...
function add_vid_after_create_hyd($cy_rec, $vid_ary,$fzx_id,$jcbz_data) {
	...
	if($cy_rs['status']>=6 || !empty($xc_value_add)){
		foreach($vid_ary as $key=>$value){
			...
			if(!$res) {
				$rowData = array();
				$sql = "SELECT * FROM `xmfa` WHERE `fzx_id`='{$fzx_id}' AND `act`='1' AND `mr`='1' AND `lxid` {$water_type_str} AND `xmid` ='{$value}'";
				$xmfa_rs= $DB->fetch_one_assoc($sql);
				if($cyd['xc_exam_value']&&strstr(','.$value.',',','.$cyd['xc_exam_value'].',')){
					$xmfa_rs['userid'] = $cyd['cy_user'];
				}
...
/*
* 全程空白项目没有water_type。传入cr_rec值，传出water_type
*/
function get_qckb_water_type ($cy_rec){
	global $DB;
	if(!empty($cy_rec['kb_sid'])){
		$yuanyang_sites = $DB->fetch_one_assoc("SELECT `water_type` FROM `sites` WHERE `id`='{$cy_rec['kb_sid']}'");
                $water_type     = $yuanyang_sites['water_type'];
        }else{
                $water_type     = 1;
        }
	return $water_type;
