...
$fzx_id=$u['fzx_id'];
// ALTER TABLE `cy_rec` ADD `show_site_name` CHAR(1) NOT NULL DEFAULT '0' COMMENT '是否在化验单显示站点名称' AFTER `site_name`;
...
if(empty($cyd['jcwc_date'])||$cyd['jcwc_date']=='0000-00-00'){
	$cyd['jcwc_date']=date("Y-m-d",strtotime("{$cyd['cy_date']}   +10   day"));
}
if($cyd['status']>='6'){
...
//导航
$csrw_title='分配检测任务 '."<span style='font-size:20px;color:red;font-weight:bold'>"."(采样单编号:".$_GET['cyd_bh'].")"."</span>";
if($cyd['status']>'5'){
	$csrw_title='修改检测任务 '."<span style='font-size:20px;color:red;font-weight:bold'>"."(采样单编号:".$_GET['cyd_bh'].")"."</span>";
}
...
// 是否允许化验员从化验单查看样品编号对照表
$sql = "SELECT `id` FROM `cy_rec` WHERE `cyd_id`='{$cyd_id}' AND `show_site_name`='1' LIMIT 1";
$show_site_name_checked = $DB->fetch_one_assoc($sql) ? 'checked' : '';
