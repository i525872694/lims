...
require_once "$rootdir/data_statistics/report_data_func.php";
if($u['userid'] == ''){
	nologin();
}
$fzx_id		= $u['fzx_id'];
...
$trade_global['js']					= array('date-time/bootstrap-datepicker.min.js','date-time/bootstrap-timepicker.min.js','boxy.js','lims/d3/d3.js',
...
if(!empty($result_set_json['all_sites'])){
	if(!empty($result_set_json['alone_sites'])){
		$sites_ids = array_unique(array_merge($sites_ids,$result_set_json['all_sites']));
	}else{
		$sites_ids = $result_set_json['all_sites'];
	}
	$alone_sites_str = '';
	$sites_ids_str = implode("','",$sites_ids);
	$sql = "select * from `sites` where `id` in ('$sites_ids_str')";
	$query = $DB->query($sql);
	$i	= 0;
	while($row = $DB->fetch_assoc($query)){
		$i++;
		$alone_sites_str	.= "<span class=\"s_float\"><label><input  name=\"all_sites[]\" field='result_set' type=\"checkbox\" value=".$row['id']." site='yes' bz='{$i}' checked />".$row['site_name']."</label></span>";
	}
}
###############站点选择区域结束###########
###########项目排序选择
...
if(!empty($gx_set_json['set_mb_name'])){
	disp("data_statistics/{$gx_set_json['set_mb_name']}");
}else{
	disp("data_statistics/any_sites_result");
}
?>