...
if($site_switch	== 'yes'){
	...
	if($gx_set_json['fc_site']	== 'yes'){//站点为分厂上传数据的时候走这里
		...
		//新版下达
		if($site_type=='0'){
			$site_type_button = <<<EOF
			<button class='btn btn-primary btn-xs' type='button' id='checkec_modi'>站点类型</button>
EOF;
			$sites_ids = array();
			$field_name = 'all_sites[]';
			$bg_info = $DB->fetch_one_assoc("select * from `baogao_list` where `id`='$_POST[set_id]'");
			$result_set = json_decode($bg_info['result_set'],true);
			$gr_ids = $result_set['alone_sites'];
			$gr_ids_str = implode("','",$gr_ids);
			$group_sql = "select * from `site_group` where `id` in ('$gr_ids_str')";
			$query = $DB->query($group_sql);
			while($row = $DB->fetch_assoc($query)){
				$sites_ids[$row['site_id']] = 1;
			}
			$sites_ids = array_keys($sites_ids);
			if(!empty($result_set['all_sites'])){
				$sites_ids = array_unique(array_merge($sites_ids,$result_set['all_sites']));
			}
			$query = $DB->query("select * from `sites` where `fzx_id`='{$fzx_id}' and `act`='1'");
			$i = 0;
			while($row = $DB->fetch_assoc($query)){
				if($i%$line_nums==0||$i==0){
					$group_site_str .= "<tr>";
				}
				$g_checked = $g_class = '';
				//判断是否选中站点
				if(in_array($row['id'],$sites_ids)){
					$g_checked	=' checked="checked" is_checked="yes" ';
					$g_class	= " class='checked-label' ";
				}
				$st_id_str  = [];
				$key_st_id = $st_site_list[$row['id']];
				foreach ($st_sid_list[$row['id']] as $key => $value) {
					if(stristr($stype[$value], ",$key_st_id,")){
						$st_id_str[]  = $stype[$value];
					}
				}
				$st_id_str  = ",".implode(',', $st_id_str).",";//该站点在该一级任务类型下所有所属类型
				$group_site_str .= "<td class='site-div' stid='$st_id_str'><label $g_class><input type=\"checkbox\" value=".$row['id']." gr_id=".$row['id']." name='sites[".$row['site_name']."][]' onclick='check_group(this)'  $g_checked />".$row['site_name']."</label></td>";
				$i++;
				if($i%$line_nums==0){
					$group_site_str .= "</tr>";
				}
...
###################项目选择区域 结束###############
echo temp("data_statistics/ajax_choose_site_value.html");
//获取父级id
