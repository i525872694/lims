	...
	$.fn.extend({
		...
		zhikong_init : function(tid,o){
			...
			$(".order_button[data-action='2'],.order_button[data-action='22']").click(function(){
				...
				modal.find(".zhikong_title").html("修改空白");
				...
				setTimeout('$("#modal_zhikong").enter_next_input({find:"[data-type='+data_type+'] input"})',500);
			})
			//自控样添加及修改
			$( [".order_button[data-action='4']",
				...
				if($.inArray(action, ['44', '88']) >= 0){
					modal.find("[name='barCode']").val(bar_code);
					...
					modal.find("[name='barCode']").val($.fn.zhikong.set.zky_name);
				}
			...
			$(".order_button[data-action='40'],.order_button[data-action='4040']").click(function(){
				...
				var span_o	= o.form.find(".hydzk[data-orid='"+orid+"']");
				var flag	= span_o.attr('data-flag');
				var zk_data = span_o.attr('data-zk-data');
				if(zk_data){
					zk_data = $.parseJSON($.parseJSON(zk_data));
				}
				(typeof zk_data === "object") || (zk_data = {});
				var data_type = "jiabiao";
				var modal = $("#modal_zhikong");
				var bar_code = o.form.find("span[data-orid='"+orid+"']").html();
				modal.find(".modal-body").hide();
				modal.find("[data-type='"+data_type+"']").show();
				// 循环zk_jb_data_default，赋值时优先使用zk_data
				for(key in zk_jb_data_default){
					if(!modal.find("[name='"+key+"']").length){
						continue;
					}
					var value = typeof zk_data[key] != "undefined" ? 
								zk_data[key] : zk_jb_data_default[key];
					modal.find("[name='"+key+"']").val(value);
				}
				if('4040'==$(this).attr('data-action')){
					modal.find(".zhikong_title").html(bar_code+"修改");
				}else if('40'==$(this).attr('data-action')){
				...
				modal.find("[name='data-type']").val(data_type);
				modal.modal("show");
				setTimeout('$("#modal_zhikong [data-type='+data_type+'] [name=v_y]").eq(0).select()',500);
				setTimeout('$("#modal_zhikong").enter_next_input({find:"[data-type='+data_type+'] input"})',500);
			})
	...
	$("#zhikong_modal_submit").click(function(){
		...
		if('kongbai'==data_type){
			if('NULL'==modal.v("xhz")){
				empty_error = true;
				modal.i("xhz").select();
				FormError.html(modal.i("xhz").parent("td").prev("td").html()+'不能为空').show();
			}else{
			...
			/*if(''==modal.v("bzz")){
				empty_error = true;
				modal.i("bzz").select();
				FormError.html(modal.i("bzz").parent("td").prev("td").html()+'不能为空').show();
			}else if(''==modal.v("bqdd")){
				empty_error = true;
				modal.i("bqdd").select();
				FormError.html(modal.i("bqdd").parent("td").prev("td").html()+'不能为空').show();
			}else*/{
				action_url += '&addZkyGs='+modal.v('addZkyGs')+'&add_all='+modal.find('[name=add_all]:checked').length+'&piHao='+modal.v('piHao')+'&biaoZhunZhi='+modal.v('bzz')+'&buQueDingDu='+modal.v('bqdd')+'&vd31='+modal.v('bzzdw')+'&vd32='+modal.v('bqdddw')+'&barCode='+modal.v('barCode');
			}
		}else if('jiabiao'==data_type){
			for(key in zk_jb_data_default){
				if(!modal.i(key).length){
					continue;
				}
				if(''==modal.v(key)){
					empty_error = true;
					modal.i(key).select();
					FormError.html(modal.i(key).parent("td").prev("td").text()+'不能为空').show();
					break;
				}else{
					action_url += '&'+key+'='+modal.v(key)
				}
			}
		}else{
			empty_error==true;
