...
var methods = {
    ...
    init : function( options ) {
        ...
        var defaults = {
            ...
            tableHeight:'300px',
            // About message
            about:'安恒集团·北京澄星科技有限公司'
        };
        // window.gEditable为外部属性，在编辑模式下开启
        window.gEditable || (window.gEditable = false);
        ...
        if (options.manualRowResize != undefined) {
            options.RowResize = options.manualRowResize;
        }
        ...
        // cellDataArray
        if(! $.fn.jexcel.defaults[id].cellDataArray){
            $.fn.jexcel.defaults[id].cellDataArray = [];
        }
        // config
        if(! $.fn.jexcel.defaults[id].config){
            $.fn.jexcel.defaults[id].config = {};
        }
        ...
        $(this).jexcel('updateSelection');
        return $.fn.jexcel.defaults[id];
    },
        ...
        for (var i = 0; i < size; i++) {
            ...
            if ($.fn.jexcel.defaults[id].columns[i].type == 'autocomplete' || $.fn.jexcel.defaults[id].columns[i].type == 'dropdown') {
                ...
                if (! $.fn.jexcel.defaults[id].columns[i].options.format) {
                    $.fn.jexcel.defaults[id].columns[i].options.format = 'YYYY-MM-DD';
                }
            }
        }

        // Number of row
        var size = $.fn.jexcel.defaults[id].data.length;

        // Preparations
        for (var i = 0; i < size; i++) {
            if (! $.fn.jexcel.defaults[id].rowHeights[i]) {
                $.fn.jexcel.defaults[id].rowHeights[i] = $.fn.jexcel.defaults[id].defaultRowHeight || '30';
            }
            // cellDataArray
            if(! $.fn.jexcel.defaults[id].cellDataArray[i]){
                $.fn.jexcel.defaults[id].cellDataArray[i] = [];
            }
            $.each($.fn.jexcel.defaults[id].columns, function(j, columns){
                if(! $.fn.jexcel.defaults[id].cellDataArray[i][j]){
                    $.fn.jexcel.defaults[id].cellDataArray[i][j] = {};
                }
                $.each(columns, function(k, v){
                    if(! $.fn.jexcel.defaults[id].cellDataArray[i][j][k]){
                        $.fn.jexcel.defaults[id].cellDataArray[i][j][k] = clone(columns[k]);
                    }
                })
            });
        }
    ...
    createTable : function() {
        ...
        $(table).prop('id', 'jexcel_table_' + id);
        ...
        if (! $('.select-wrap').length) {
            ...
            $(corner).prop('class', 'select-wrap');
            $(corner).prop('id', 'select-wrap');
            $(corner).append('<div class="selection-board"><div class="selection-area"></div><div class="select-area"><div id="jexcel_corner" class="jexcel_corner"></div></div><div class="active-area"></div><div class="formula-select-area"></div></div>');
            ...
            $.fn.jexcel.resizeRow = null;
            $.fn.jexcel.firstClickCell = null;
            $.fn.jexcel.lastClickCell = null;
                    ...
                    if ($(table).is('.jexcel')) {
                        ...
                        if (o) {
                            ...
                            if (typeof($.fn.jexcel.defaults[$.fn.jexcel.current].contextMenu) == 'function') {
                                contextMenuContent = $.fn.jexcel.defaults[$.fn.jexcel.current].contextMenu(e);
                            } else {
                                // Default context menu for the columns
                                if ($(e.target).parent().parent().is('thead')) {
                                    if($.fn.jexcel.defaults[$.fn.jexcel.current].columnSorting == true){
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('orderBy', " + o[1] + ", 0)\">递增排序 <span></span></a>";
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('orderBy', " + o[1] + ", 1)\">递减排序 <span></span></a><hr>";
                                    }
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertColumn == true) {
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertColumn', 1, null, " + o[1] + ")\">插入一列<span></span></a>";
                                    }
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteColumn == true) {
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('deleteColumn'," + o[1] + ")\">删除当前列<span></span></a>";
                                    }
                                    contextMenuContent += "<hr><a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('download')\">另存为...<span>Ctrl + S</span></a>";
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].about) {
                                        contextMenuContent += "<a onclick=\"alert('" + $.fn.jexcel.defaults[$.fn.jexcel.current].about + "')\">关于<span></span></a>";
                                    }
                                } else if ($(e.target).parent().parent().is('tbody')) {
                                    // Default context menu for the rows
                                    if($(e.target).hasClass('jexcel_label')){
                                        if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertColumn == true) {
                                            contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertColumn', 1, null, " + o[1] + ")\">插入一列<span></span></a>";
                                        }
                                        if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowInsertRow == true) {
                                            contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('insertRow', 1, " + o[1] + ")\">插入一行<span></span></a><hr>";
                                        }
                                        if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteRow == true) {
                                            contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('deleteRow'," + o[1] + ")\">删除当前行<span></span></a><hr>";
                                        }
                                    }else{
                                        // 合并拆分单元格
                                        var selRange = $($.fn.jexcel.defaults[$.fn.jexcel.current].div).jexcel("getSelectedRange");
                                        if(!selRange.isSingle()){
                                            selRange.isSingleMergeCell() || (contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('mergeCell')\">合并单元格<span></span></a>");
                                            selRange.hasMergeCellsInSelectedRange() && (contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('unmergeCell')\">拆分单元格<span></span></a>");
                                        }
                                    }
                                    contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('download')\">另存为...<span>Ctrl + S</span></a>";
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].about) {
                                        contextMenuContent += "<a onclick=\"alert('" + $.fn.jexcel.defaults[$.fn.jexcel.current].about + "')\">关于<span></span></a>";
                                    }
                                }
                    ...
                    if ($(table).is('.jexcel')) {
                        ...
                        if ($(e.target).parent().parent().is('tbody')) {
                            ...
                            if ($(e.target).is('.jexcel_label')) {
                                if ($.fn.jexcel.defaults[$.fn.jexcel.current].rowDrag == true && $(e.target).outerWidth() - e.offsetX < 8) {
                                ...
                                } else if ($.fn.jexcel.defaults[$.fn.jexcel.current].rowResize == true && $(e.target).outerHeight() - e.offsetY < 8) {
                                    var o = $(e.target).parent().prop('id');
                                    if (o) {
                                        o = o.split('-');
                                        // Resize helper
                                        ...
                                        // Border indication
                                        $(table).parent().find('#row-' + o[1] + ' td').addClass('resizingRow');
                                    ...
                                    // Update selection
                                    $('#' + $.fn.jexcel.current).jexcel('updateSelection', o1, o2, 2);
                                ...
                                if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                    ...
                                    // 在选择区域的单元格右键
                                    if($(e.target).hasClass('highlight') && $.fn.jexcel.lastClickCell && e.button == 2){
                                        selCel1 = $.fn.jexcel.firstClickCell;
                                        selCel2 = $.fn.jexcel.lastClickCell;
                                        // 在选区右键不更新选框
                                        // $('#' + $.fn.jexcel.current).jexcel('updateSelection', selCel1, selCel2);
                                    }else{
                                        // 记录本次点击的单元格
                                        $.fn.jexcel.firstClickCell = $(e.target);
                        ...
                        if (! $(e.target).parents('.jexcel').length && ! window.gEditable) {
            ...
            $.fn.jexcel.mouseUpControls = function (e) {
                ...
                if (e.target.id == 'jexcel_arrow') {
                    ...
                    if ($.fn.jexcel.resizeColumn) {
                        ...
                        // On resize
                        if (typeof($.fn.jexcel.defaults[$.fn.jexcel.current].onresizecol) == 'function') {
                            $.fn.jexcel.defaults[$.fn.jexcel.current].onresizecol($(this), $.fn.jexcel.resizeColumn.column, newWidth, $.fn.jexcel.resizeColumn.width);
                        }
                    ...
                    // Update cell size
                    if ($.fn.jexcel.resizeRow) {

                        // rows to be updated
                        var changedRow = $('#' + $.fn.jexcel.current + ' tbody').find('#row-' + $.fn.jexcel.resizeRow.row);
                        var changedrows = $('#' + $.fn.jexcel.current + ' tbody').find('#row-' + $.fn.jexcel.resizeRow.row + ' td');

                        // New height
                        var newHeight = parseInt($(changedRow).height());

                        // On resize
                        if (typeof($.fn.jexcel.defaults[$.fn.jexcel.current].onresizerow) == 'function') {
                            $.fn.jexcel.defaults[$.fn.jexcel.current].onresizerow($(this), $.fn.jexcel.resizeRow.row, newHeight, $.fn.jexcel.resizeRow.height);
                        }

                        // Update cells
                        $(changedrows).removeClass('resizingRow');
                        $(changedRow).prop('height', newHeight + 'px');

                        // Update container
                        $.fn.jexcel.defaults[$.fn.jexcel.current].rowHeights[$.fn.jexcel.resizeRow.row] = newHeight;
                    ...
                    // 记录最后一次点击的单元格
                    if ($(e.target).parent().parent().is('tbody') && !$(e.target).is('.jexcel_label')){
                        var c = $(e.target).prop('id').split('-');
                        var o = $.fn.jexcel.firstClickCell.prop('id').split('-');
                        var d = $.fn.jexcel.lastClickCell.prop('id').split('-');
                        // 不在选区右键时重新记录点击的单元格
                        if(parseInt(c[0]) < parseInt(o[0]) || parseInt(c[0]) > parseInt(d[0])){
                            if(parseInt(c[1]) < parseInt(o[1]) || parseInt(c[1]) > parseInt(d[1])){
                                $.fn.jexcel.lastClickCell = $(e.target);
                            }
                        }
                    }
            ...
            $.fn.jexcel.mouseMoveControls = function (e) {
                if ($.fn.jexcel.current) {
                    ...
                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].rowResize == true) {
                        // Resizing is ongoing
                        if ($.fn.jexcel.resizeRow) {
                           var height = e.pageY - $.fn.jexcel.resizeRow.mousePosition;

                           if ($.fn.jexcel.resizeRow.height + height > 0) {
                               $('#' + $.fn.jexcel.current).jexcel('setHeight', $.fn.jexcel.resizeRow.row, $.fn.jexcel.resizeRow.height + height);
                           }
                        } else {
                            // Header found
                            if ($(e.target).is('.jexcel_label')) {
                                // Update cursor
                                if ($(e.target).outerHeight() - e.offsetY < 8 && $(e.target).is('.jexcel_label')) {
                                    $(e.target).css('cursor', 'row-resize');
                                } else {
                                    if ($(e.target).css('cursor') == 'row-resize') {
                                        $(e.target).css('cursor', '');
                                    }
                                }
                            }
                            ...
                            if (e.metaKey && ! e.shiftKey && ! e.ctrlKey) {
                                ...
                                if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                    if (e.which == 65) {
                                        ...
                                        $('#' + $.fn.jexcel.current).jexcel('saveData');
                                        // Prevent page selection
                                        e.preventDefault();
                                    } else if (e.which == 89) {
                                        // Ctrl + Y
    ...
    // ######### 自定义函数 start
    // 获取单元格名称
    getLetter : function(col, row){
        return $.fn.jexcel('getColumnName', col) + (parseInt(row) + 1);
    },
    // 获取合并的单元格
    getSpans: function(){
        // Id
        ...
        // 获取合并单元格中显示和隐藏的单元格
        var showSpans = [], hideSpans = [],spanArr = {};
        for(var i = 0; i < spans.length; i++){
            for(var j = 0 ; j < spans[i].rowCount; j++){
                for(var k = 0 ; k < spans[i].colCount; k++){
                    var col = spans[i].col+k;
                    var row = spans[i].row+j;
                    var cell = $.fn.jexcel('getLetter', col, row);
                    if(j==0&&k==0){
                        showSpans.push(cell);
                        spanArr[cell] = spans[i];
                    }else{
                        hideSpans.push(cell);
                    }
                }
        ...
        return {showSpans: showSpans, hideSpans: hideSpans, info: spanArr};
    },
    // 更新单元格合并信息
    updateSpans : function(){
        ...
        $.fn.jexcel.defaults[id].spans = [];
        $(this).find('tbody td[data-merge=mergeItem]').each(function(){
            var or = $(this).prop('id').split('-');
            var spans = {
                col: parseInt(or[0]),
                row: parseInt(or[1]),
                colCount: parseInt($(this).prop('colspan')),
                rowCount: parseInt($(this).prop('rowspan'))
            };
            $.fn.jexcel.defaults[id].spans.push(spans);
            $.fn.jexcel.defaults[id].cellDataArray[or[1]][or[0]].spans = spans;
        });
    },
    // 根据配置信息合并单元格
    mergeCells : function(spans){
        // Id
        var id = $(this).prop('id');
        if(typeof spans == "undefined"){
            var spans = $.fn.jexcel.defaults[id].spans;
        }
        for(var n=0; n<spans.length; n++){
            var cellId = spans[n].col+"-"+spans[n].row;
            for (var i = 0; i < spans[n].rowCount; i++) {
                for (var j = 0; j < spans[n].colCount; j++) {
                    var row = i + parseInt(spans[n].row);
                    var col = j + parseInt(spans[n].col);
                    var cell = $.fn.jexcel('getLetter', col, row);
                    if(col == spans[n].col && row == spans[n].row){
                        $(this).jexcel('getCell', cell)
                        .attr('data-merge', 'mergeItem').removeClass('hide')
                        .prop('rowspan', spans[n].rowCount).prop('colspan', spans[n].colCount);
                    }else{
                        $(this).jexcel('getCell', cell)
                            .prop('rowspan', 1).prop('colspan', 1)
                            .attr('data-merge', cellId).addClass('hide');
                        delete $.fn.jexcel.defaults[id].cellDataArray[row][col].spans;
                    }
                }
            }
        }
    ...
    // 合并单元格
    mergeCell : function(o, d){
        if(!o || !d){
            var o = $.fn.jexcel.firstClickCell;
            var d = $.fn.jexcel.lastClickCell;
        }
        var id = $(this).prop('id');
        var or = $(o).prop('id').split('-');
        var de = $(d).prop('id').split('-');
        var spans = [
            {
                col: parseInt(or[0]),
                row: parseInt(or[1]),
                colCount: parseInt(de[0]) - parseInt(or[0]) + 1,
                rowCount: parseInt(de[1]) - parseInt(or[1]) + 1
            }
        ];
        $('#' + $.fn.jexcel.current).jexcel('mergeCells', spans);
        ...
        // 更新单元格合并信息
        $('#' + id).jexcel('updateSpans');
        // 更新选区
        $('#' + id).jexcel('updateSelection', o, o);
    },
    // 拆分单元格
    unmergeCell : function(o, d){
        if(!o || !d){
            var o = $.fn.jexcel.firstClickCell;
            var d = $.fn.jexcel.lastClickCell;
        }
        ...
        var or = $(o).prop('id').split('-');
        var de = $(d).prop('id').split('-');
        for (var i = parseInt(or[0]); i <= parseInt(de[0]); i++) {
            for (var j = parseInt(or[1]); j <= parseInt(de[1]); j++) {
                var cellId = i + '-' + j;
                $(this).find('td[id='+cellId+'],td[data-merge='+cellId+']').each(function(){
                    var col = $(this).prop('id').split('-')[0];
                    var row = $(this).prop('id').split('-')[1];
                    delete $.fn.jexcel.defaults[id].cellDataArray[row][col].spans;
                    $(this).prop('rowspan', 1).prop('colspan', 1).removeAttr('data-merge').removeClass('hide');
                });
            }
        }
        // Keeping history of changes
        $(this).jexcel('setHistory', null, {
            type:'unmergeCell',
            selection: [o, d],
            spans: $.fn.jexcel.defaults[id].spans.slice()
        });
        // 更新单元格合并信息
        $('#' + id).jexcel('updateSpans');
        // 更新选区
        $('#' + id).jexcel('updateSelection', o, d);
    },
    showHeader: function(){
        $(this).jexcel("showRowHeader");
        $(this).jexcel("showColumnHeader");
    },
    hideHeader: function(){
        $(this).jexcel("hideRowHeader");
        $(this).jexcel("hideColumnHeader");
    },
    showRowHeader: function(){
        // Id
        var id = $(this).prop('id');
        var options = $.fn.jexcel.defaults[id];
        $(this).find("tbody td.jexcel_label").show();
        if(options.showColumnHeader){
            $(this).find("thead td.jexcel_label").show();
        }else{
            $(this).find("thead td.jexcel_label").hide();
        }
    },
    hideRowHeader: function(){
        // Id
        var id = $(this).prop('id');
        var options = $.fn.jexcel.defaults[id];
        $(this).find("tbody td.jexcel_label").hide();
        if(options.showColumnHeader){
            $(this).find("thead td.jexcel_label").show();
        }else{
            $(this).find("thead td.jexcel_label").hide();
    ...
    showColumnHeader: function(){
        ...
        var id = $(this).prop('id');
        var options = $.fn.jexcel.defaults[id];
        $(this).find('thead.jexcel_label td:gt(0)').each(function(i){
            // Default header cell properties
            var title = options.colHeaders[i];
            // Header
            var header = title || $.fn.jexcel('getColumnName', i);
        ...
        $(this).find("thead.jexcel_label").removeClass('noborder');
    },
    hideColumnHeader: function(){
        $(this).find("thead td").html('');
        $(this).find("thead.jexcel_label").addClass('noborder');
    },
    // jExcel工作表创建完后进行更新
    updateAllCells : function(){
        ...
        // 加载自定义样式css样式
        options.namedStyles || (options.namedStyles = []);
        $("#jExcel-css").length || $("body").append('<style id="jExcel-css" type="text/css"></style>');
        for(className in options.namedStyles){
            var styleLine = '#'+id+' .jexcel > tbody > tr > td.'+className+JSON.stringify(options.namedStyles[className]);
            // 将json格式的css样式转译成css样式文本格式并插入自定义style中
            $("#jExcel-css").append(
                styleLine.replace(/\"/g, "").replace(/,/g, ";").replace(/([0-9];)/g, "$1,").replace(/;,/g, ",")
            );
        }
        // 合并单元格
        if(options.spans){
            $(this).jexcel("mergeCells", options.spans);
        }
        // 旧版的数据绑定更新成新版的
        if($.fn.jexcel.defaults[id].config.bindInfo || $.fn.jexcel.defaults[id].config.otherBind || $.fn.jexcel.defaults[id].config.tdBind || $.fn.jexcel.defaults[id].config.vdBind){
            // 更新绑定信息
            var allBind = $.extend({}, $.fn.jexcel.defaults[id].config.tdBind, $.fn.jexcel.defaults[id].config.vdBind);
            for(i in allBind){
                $.fn.jexcel.defaults[id].cellDataArray[allBind[i][1]][allBind[i][0]].link = [i, ''];
            }
            delete $.fn.jexcel.defaults[id].config.tdBind;
            delete $.fn.jexcel.defaults[id].config.vdBind;
            delete $.fn.jexcel.defaults[id].config.bindInfo;
            delete $.fn.jexcel.defaults[id].config.otherBind;
        }
        // 窗口变化时，自适应变换选区
        $.fn.jexcel.windowResize = function(){
            if($.fn.jexcel.current){
                var id = $.fn.jexcel.current;
                $($.fn.jexcel.defaults[id].div).jexcel("resize");
            }
        }
        $(window).resize($.fn.jexcel.windowResize);
    ...
    setStyle : function(opt){
        for (var i = 0; i < opt.length; i++) {
            $(this).find(opt[i].e).attr("class", opt[i].class);
            $(this).find(opt[i].e).attr("style", opt[i].style);
        }
    },
    // 复制样式
    copyStyle: function(o, d){
        var c = $(o).attr("class").match(/__builtInStyle[0-9]+/i);
        var s = {
            class: c ? c[0] : '',
            style: $(o).attr("style") || ''
        };
        $(d).each(function(){
            s.class && $(this).addClass(s.class);
            s.style && $(this).attr("style", s.style);
        });
    },
    // 保存
    saveData: function(){
        var id = $(this).prop('id');
        var o = $.fn.jexcel.defaults[id];
        // if(!o || o.saveIndex == o.historyIndex){
        //     return false;
        // }
        $(o.div).jexcel("resetSelection");
        if(typeof o.saveData != "function"){
            // $(o.div).jexcel('download');
            return;
        }
        // 自定义保存函数
        var newData = {};
        var keys = ["title", "config", "spans", "colWidths", "rowHeights", "data"];
        // 更新单元格合并信息
        $(o.div).jexcel('updateSpans');
        // 默认数据
        ...
        // css类样式
        newData['namedStyles'] = {};
        newData['cellDataArray'] = [];
        // V是具体样式字符串,K是类名称,判断是否已经定义过该样式,避免重复定义相同样式
        var bsV = [];
        var bsK = [];

        for (var i = 0; i < newData['data'].length; i++) {
            newData['cellDataArray'][i] = [];
            for (var j = 0; j < newData['data'][i].length; j++) {
                var e = $(o.div).find("td[id=" + j + '-' + i + "]");
                newData['cellDataArray'][i][j] = {};
                if(o['cellDataArray'][i] && typeof o['cellDataArray'][i][j] == 'object' && isNaN(o['cellDataArray'][i][j].length)){
                    newData['cellDataArray'][i][j] = clone(o['cellDataArray'][i][j]);
                    // 必须删除单元格自定义样式里面定义的合并单元格信息,此信息仅仅在历史操作记录中使用
                    delete newData['cellDataArray'][i][j]['spans'];
                }
                // 获取行内样式
                styleObj = {};
                if($(e).attr('style')){
                    var styleArr = $(e).attr('style').split(";");
                    for (var sk = 0; sk < styleArr.length; sk++) {
                        if(styleArr[sk]){
                            styleObj[styleArr[sk].split(":")[0]] = styleArr[sk].split(":")[1];
                        }
                    }
                }
                bs = '';
                // 获取之前的自定义样式
                var bss = $(e).attr("class").match(/__builtInStyle[0-9]+/i);
                bss = bss ? o['namedStyles'][bss[0]] : {};
                if(JSON.stringify(bss) != "{}" || JSON.stringify(styleObj) != "{}"){
                    // 将行样式与之前自定义样式合并重建样式
                    styleObj = $.extend(bss, styleObj);
                    // 定义当前单元格的builtInStyle类名
                    bs = bsK[$.inArray(JSON.stringify(styleObj), bsV)];
                    if(!bs){
                        // 定义css类名称
                        var bs = '__builtInStyle' + s++;
                        bsK.push(bs);
                        bsV.push(JSON.stringify(styleObj));
                        newData['namedStyles'][bs] = styleObj;
                    }
                }
                newData['cellDataArray'][i][j]['style'] = bs;
            }
        }
        newData.rowCount = newData.data.length;
        newData.columnCount = newData.data[0].length;
        console.log(newData)
        o.saveData(newData);
        $.fn.jexcel.defaults[id].saveIndex = $.fn.jexcel.defaults[id].historyIndex;
        // 恢复选区
        $(this).jexcel('updateSelection', $.fn.jexcel.firstClickCell, $.fn.jexcel.lastClickCell);
    },
    // 设置样式
    resize : function(opt){
        var id = $(this).prop("id");
        if($(this).jexcel('getSelection')){
            $(this).jexcel('updateSelection', $.fn.jexcel.selection[0], $.fn.jexcel.selection[1]);
        }
    },
    // 获取选中单元格
    getSelectedRange: function(){
        var id = $.fn.jexcel.current;
        var e = $.fn.jexcel.defaults[id];
        n = $(this).jexcel('getSelection');
        if(!n){
            return false;
        }
        a = n[0].prop('id').split('-')[0],
        o = n[0].prop('id').split('-')[1],
        s = n[1].prop('id').split('-')[0],
        i = n[1].prop('id').split('-')[1];
        return {
            from: {
                row: o,
                col: a
            ...
            isSingleMergeCell: function() {
                var t = $(e.div).find('tbody td.highlight').not(".hide");
                return t.length == 1 && (o != i || a != s);
            },
            hasMergeCellsInSelectedRange: function() {
                return $(e.div).find('tbody td.highlight[data-merge=mergeItem]').not(".hide").length;
            }
        }
    },
    getActiveCell: function(f){
        if($.fn.jexcel.firstClickCell){
            var o = $.fn.jexcel.firstClickCell.prop("id").split("-");
            return f ? $.fn.jexcel.firstClickCell : [o[0], o[1]];
        }
        return false;
    },
    // ######### 自定义函数 end
    ...
    setData : function(data, ignoreSpare) {
        ...
        // Create history track array
        $.fn.jexcel.defaults[id].history = [];
        $.fn.jexcel.defaults[id].saveIndex = -1;
        $.fn.jexcel.defaults[id].historyIndex = -1;
        ...
        for (j = 0; j < $.fn.jexcel.defaults[id].data.length; j++) {
            ...
            for (i = 0; i < $.fn.jexcel.defaults[id].colHeaders.length; i++) {
                ...
                records.push({
                    ...
                    oldValue: '',
                    cellData: $.fn.jexcel.defaults[id].cellDataArray[j][i]
                });
            }
        ...
        if ($(cell).hasClass('readonly') == true) {
            ...
            if (options.columns[position[0]].editor) {
                ...
                if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'hidden') {
                    ...
                    // Bind mask 适用lims重新开发此插件
                    var cellDataArray = clone(options.cellDataArray[position[1]][position[0]]);
                    if (! $.fn.limsMask) {
                        console.error('Jexcel: it was not possible to load the mask plugin.');
                    } else {
                        $(cell).limsMask(editor, cellDataArray)
                    }
    ...
    updateCell : function(v, force) {
        ...
        if ($(v.cell).hasClass('readonly') == true && force == false) {
            ...
            if (options.columns[position[0]].editor) {
                ...
                if (options.columns[position[0]].type == 'checkbox') {
                    ...
                    if (value.substr(0,1) == '=') {
                        ...
                        if(options.cellDataArray){
                            var cellDataArray = clone(options.cellDataArray[v.row][v.col]);
                            if (undefined !== cellDataArray.mask && $.fn.limsMask) {
                                val = $(v.cell).limsMask(val, cellDataArray);
                            }
                        }
            ...
            if(v.cellData){
                if(v.cellData.style){
                    $(v.cell).addClass(v.cellData.style);
                }
                if(v.cellData.lineStyle){
                    $(v.cell).attr('style', v.cellData.lineStyle);
                }
                if(v.cellData.spans){
                    var spans = clone(v.cellData.spans);
                    $(this).jexcel('unmergeCell', v.cell, v.cell);
                    v.cellData.spans = clone(spans);
                    $(this).jexcel('mergeCells', [v.cellData.spans])
                }
            }
    ...
    updateSelection : function(o, d, origin) {
        ...
        if(
            o && d &&
            !($(o.selector).length && $(d.selector).length) &&
            !($(main).find(o).length && $(main).find(d).length)
        ){
            o = d = null;
        }
        ...
        if (o && d) {
            ...
            px = Math.min(parseInt(or[0]), parseInt(de[0]));
            ux = Math.max(parseInt(or[0]), parseInt(de[0]));
            py = Math.min(parseInt(or[1]), parseInt(de[1]));
            uy = Math.max(parseInt(or[1]), parseInt(de[1]));
            if(1 == origin || 2 == origin){
                // 这是在header栏选择行列
                // Redefining styles
                for (var i = px; i <= ux; i++) {
                    for (var j = py; j <= uy; j++) {
                        // Select markers
                        var curCell = $(this).find('[id=' + i + '-' + j + ']').addClass('highlight');
                ...
                // [id=0-0]是初始位置
                var xyCell = $(this).find('#0-0');
                if(origin == 1){
                    var lHeader = $(this).find('#col-' + px);
                    var rHeader = $(this).find('#col-' + ux);
                    var botCell = $(this).find('tbody');
                    var l = parseInt(lHeader.offset().left);
                    var t = parseInt(xyCell.offset().top);
                    var b = parseInt(botCell.offset().top) + parseInt(botCell.outerHeight());
                    var r = parseInt(rHeader.offset().left) + parseInt(rHeader.outerWidth());
                }else{
                    var tHeader = $(this).find('#row-' + py + ' .jexcel_label');
                    var bHeader = $(this).find('#row-' + uy + ' .jexcel_label');
                    var rHeader = $(this).find('tbody');
                    var l = parseInt(tHeader.offset().left) + parseInt(tHeader.outerWidth());
                    var t = parseInt(tHeader.offset().top);
                    var b = parseInt(bHeader.offset().top) + parseInt(bHeader.outerHeight());
                    var r = parseInt(rHeader.offset().left) + parseInt(rHeader.outerWidth())
                }
            }else{
                // 如果当前单元格是被合并而隐藏的单元格需要找出合并的单元位置
                var cellId = px + '-' + py;
                if(rows.find('[id='+cellId+']').hasClass('hide')){
                    var mergeItem = rows.find('[id='+cellId+']').data('merge').split('-');
                    px = parseInt(mergeItem[0]);
                    py = parseInt(mergeItem[1]);
                }
                var uux = ux;
                var uuy = uy;
                // 取出最大的行列
                for (var i = px; i <= uux; i++) {
                    for (var j = py; j <= uuy; j++) {
                        var cellId = i + '-' + j;
                        var colspan = parseInt(rows.find('[id='+cellId+']').prop('colspan'))-1 || 0;
                        var rowspan = parseInt(rows.find('[id='+cellId+']').prop('rowspan'))-1 || 0;
                        // 列循环不能跳过，必须遍历每一列
                        uux = Math.max(i + colspan, uux);
                        // 本列被合并的行直接continue，用j+=
                        uuy = Math.max(j += rowspan, uuy);
                ...
                // postion
                var xyCell = $(this).find('#0-0');
                var ltCell = $(this).find('#'+px+'-'+py) || xyCell;
                var t = parseInt(ltCell.offset().top), b = t;
                var l = parseInt(ltCell.offset().left), r = l;

                // Redefining styles
                for (var i = px; i <= ux; i++) {
                    for (var j = py; j <= uy; j++) {
                        // Select markers
                        var curCell = $(this).find('[id=' + i + '-' + j + ']');
                        if(!curCell.length){
                            console.warn(curCell, "不存在")
                            return;
                        }
                        curCell.addClass('highlight');
                        b = Math.max(b, parseInt(curCell.offset().top) + parseInt(curCell.outerHeight()));
                        r = Math.max(r, parseInt(curCell.offset().left) + parseInt(curCell.outerWidth()));
                ...
                // [id=0-0]是初始位置
                (t < parseInt(xyCell.offset().top)) && (t = parseInt(xyCell.offset().top));
                (l < parseInt(xyCell.offset().left)) && (l = parseInt(xyCell.offset().left));
            }
            ...
            $(".active-area").css({
                ...
                top: parseInt(o.offset().top) + 'px',
                left: parseInt(o.offset().left) + 'px'
            }).show();
        ...
        if (o && d) {
            $(".select-wrap").show();
            o = $(this).find('[id=' + px + '-' + py +']');
            d = $(this).find('[id=' + ux + '-' + uy +']');
            $.fn.jexcel.firstClickCell = o;
            $.fn.jexcel.lastClickCell = d;
    ...
    getSelectedCells : function() {
        // Get selected cells
        if ($.fn.jexcel.current) {
            return $('#' + $.fn.jexcel.current).find('tbody td.highlight').not(".hide");
        }
    },
    ...
    updateCornerSelection : function(current) {
        ...
        // Remove selection
        $(this).find('tbody td').removeClass('selection');

        // Get selection
        var selection = $(this).find('tbody td.highlight').not(".hide");

        // Get elements first and last
        var s = $(selection[0]).prop('id').split('-');
        var d = $(selection[selection.length - 1]).prop('id').split('-');
        d[0] = parseInt(d[0]) + parseInt($(selection[selection.length - 1]).prop("colspan")) - 1;
        d[1] = parseInt(d[1]) + parseInt($(selection[selection.length - 1]).prop("rowspan")) - 1;

        // Get current
        var c = $(current).prop('id').split('-');
        c[0] = parseInt(c[0]) + parseInt($(current).prop("colspan")) - 1;
        c[1] = parseInt(c[1]) + parseInt($(current).prop("rowspan")) - 1;
        ...
        if($.fn.jexcel.defaults[id].showColumnHeader && $.fn.jexcel.defaults[id].showRowHeader){
            var l = $("#col-"+px).offset().left;
            var t = $("#row-"+py).offset().top;
            var r = parseInt($("#col-"+ux).offset().left) + parseInt($("#col-"+ux).outerWidth());
            var b = parseInt($("#row-"+uy).offset().top) + parseInt($("#row-"+uy).outerHeight());
        }else{
            // 左上右下
            var ltCell = $(this).find('#' + px + '-' + py);
            var rbCell = $(this).find('#' + ux + '-' + uy);
            if(!$(ltCell).length || !$(rbCell).length || $(ltCell).hasClass('hide') || $(rbCell).hasClass('hide')){
                $(selection).removeClass('selection');
                $('.selection-area').hide();
                return;
            }
            var l = parseInt(ltCell.offset().left);
            var t = parseInt(ltCell.offset().top);
            var r = parseInt(rbCell.offset().left) + parseInt(rbCell.outerWidth());
            var b = parseInt(rbCell.offset().top) + parseInt(rbCell.outerHeight());
        }
        ...
        if (options.allowInsertColumn == true) {
            ...
            if (direction == 1) {
                ...
                for (var col = (numOfColumns + columnNumber - 1); col >= columnNumber; col--) {
                    ...
                    var className = options.colHeaderClasses[col];
                    className += (options.columns[col].type == 'hidden') ? ' hide' : '';
                    td = '<td width="' + options.colWidths[col] + '" align="' + options.colAlignments[col] + '" title="' + title + '" class="' + options.className + '">' + header + '</td>';
                ...
                for (var row = 0; row < options.data.length; row++) {
                    ...
                    var currentData2 = $.fn.jexcel.defaults[id].cellDataArray[row].splice(columnNumber);
                    ...
                    for (var col = (numOfColumns + columnNumber - 1); col >= columnNumber; col--) {
                        ...
                        $.fn.jexcel.defaults[id].cellDataArray[row][col] = data2[row] ? data2[row] : {};
                        ...
                        records.push({
                            ...
                            oldValue: '',
                            cellData: $.fn.jexcel.defaults[id].cellDataArray[row][col]
                        });
                    }

                    // Copy the data back to the main data
                    Array.prototype.push.apply($.fn.jexcel.defaults[id].data[row], currentData);
                    Array.prototype.push.apply($.fn.jexcel.defaults[id].cellDataArray[row], currentData2);
                }
            } else {
                ...
                for (var col = columnNumber; col < numOfColumns + columnNumber; col++) {
                    ...
                    var className = options.colHeaderClasses[col];
                    className += (options.columns[col].type == 'hidden') ? ' hide' : '';
                    td = '<td width="' + options.colWidths[col] + '" align="' + options.colAlignments[col] + '" title="' + title + '" class="' + className + '">' + header + '</td>';
                ...
                for (var row = 0; row < options.data.length; row++) {
                    ...
                    for (var col = columnNumber; col < numOfColumns + columnNumber; col++) {
                        ...
                        // 解决存在单元格合并时的情况
                        var merge = $(referenceColumn).attr("data-merge");
                        if(merge != undefined){
                            if('mergeItem' == merge){
                                $(this).find("td[data-merge="+col+"-"+row+"]").attr("data-merge", (parseInt(col)+1)+"-"+row);
                            }else{
                                if(col > merge.split("-")[0] ){
                                    $(referenceColumn).prev().attr("data-merge", merge).addClass("hide");
                                }
                                if(row == merge.split("-")[1]){
                                    // 当前行是合并单元格所在行时相应的增加colspan的值
                                    $(this).find("td[id="+merge+"]").each(function(){
                                        $(this).prop("colspan", parseInt($(this).prop("colspan"))+1)
                                    });
                                }
                            }
                        }
    ...
    insertRow : function(mixed, rowNumber) {
        ...
        if (options.allowInsertRow == true) {
            ...
            var currentData2 = $.fn.jexcel.defaults[id].cellDataArray.splice(rowNumber);
            ...
            var createRow = function() {
                ...
                for (var col = 0; col < $.fn.jexcel.defaults[id].colHeaders.length; col++) {
                    ...
                    // 解决存在单元格合并时的情况
                    var merge = $(main).find("td[id="+col+'-'+row+"]").attr("data-merge");
                    if(merge != undefined){
                        if('mergeItem' == merge){
                            $(main).find("td[data-merge="+col+"-"+row+"]").attr("data-merge", col+"-"+(parseInt(row)+1));
                        }else{
                            if(row > merge.split("-")[1] ){
                                var prevTD = $(main).find("td[id="+col+"-"+(row-1)+"]");
                                if(prevTD.length && $(prevTD).attr("data-merge")){
                                    var newMerge = ('mergeItem' == $(prevTD).attr("data-merge")) ? $(prevTD).prop("id") : $(prevTD).attr("data-merge");
                                    $(tr).find("td[id="+col+"-"+row+"]").attr("data-merge", newMerge).addClass("hide");
                                }
                            }
                            if(col == merge.split("-")[0]){
                                $(main).find("td[id="+merge+"]").each(function(){
                                    $(this).prop("rowspan", parseInt($(this).prop("rowspan"))+1)
                                });
                            }
                        }
                    }
            ...
            Array.prototype.push.apply($.fn.jexcel.defaults[id].cellDataArray, currentData2);
            ...
            if (options.data[0].length > 1) {
                if (parseInt(columnNumber) > -1) {
                    ...
                    for (var row = 0; row < $.fn.jexcel.defaults[id].data.length; row++) {
                        ...
                        data2 = options.cellDataArray[row].splice(columnNumber, numOfColumns);
                    ...
                    for (var col = columnNumber; col < columnNumber + numOfColumns; col++) {
                        ...
                        $(this).find('tbody .c' + col + '[data-merge]').each(function(){
                            // 解决存在单元格合并时的情况
                            var merge = $(this).attr("data-merge");
                            if(merge && 'mergeItem' == merge){
                                if($(this).prop("colspan") > 1){
                                    $(this).next().attr("data-merge", "mergeItem").prop("colspan", $(this).prop("colspan")-1)
                                    .prop("rowspan", $(this).prop("rowspan")).removeClass("hide");
                                     $(main).find("td[data-merge="+col+"-"+row+"]").attr("data-merge", $(this).next().prop("id"));
                                }
                            }else if(merge && $(this).prop("id").split("-")[1] == merge.split("-")[1]){
                                var spans = {
                                    col: merge.split("-")[0],
                                    row: merge.split("-")[1],
                                    colCount: $(main).find("td[id="+merge+"]").prop('colspan'),
                                    rowCount: $(main).find("td[id="+merge+"]").prop('rowspan')
                                };
                                if(spans.colCount > 1 || spans.rowCount > 1){
                                    records2[col-columnNumber][$(this).prop("id").split("-")[1]].spans = spans;
                                }
                                $(main).find("td[id="+merge+"]").each(function(){
                                    $(this).prop("colspan", parseInt($(this).prop("colspan"))-1)
                                });
                            ...
                            // 记录删除单元格的行内样式
                            if($(this).attr("style")){
                                records2[col-columnNumber][$(this).prop("id").split("-")[1]].lineStyle = $(this).attr("style");
                            }
                        });
                ...
                if ($.fn.jexcel.ignoreEvents != true) {
                    if (typeof(options.ondeletecolumn) == 'function') {
                        options.ondeletecolumn($(this), columnNumber, numOfColumns, records, records2);
                    }
                }
            } else {
            ...
            if (options.data.length > 1) {
                if (parseInt(rowNumber) > -1) {
                    ...
                    for (var row = rowNumber; row < rowNumber + numOfRows; row++) {
                        ...
                        records2[row] = $.fn.jexcel.defaults[id].cellDataArray[row];
                        ...
                        $(line).find("td[id]").each(function(){
                            // 解决存在单元格合并时的情况
                            var merge = $(this).attr("data-merge");
                            if(merge && 'mergeItem' == merge){
                                if($(this).prop("rowspan") > 1){
                                    var cellId = $(this).prop("id").split("-");
                                    var nextTD = $(main).find("td[id="+cellId[0]+"-"+(parseInt(cellId[1])+1)+"]");
                                    $(nextTD).attr("data-merge", "mergeItem").prop("rowspan", $(this).prop("rowspan")-1)
                                    .prop("colspan", $(this).prop("colspan")).removeClass("hide");
                                     $(main).find("td[data-merge="+cellId[0]+"-"+cellId[1]+"]").attr("data-merge", $(this).next().prop("id"));
                                }
                            }else if(merge && $(this).prop("id").split("-")[0] == merge.split("-")[0]){
                                var spans = {
                                    col: merge.split("-")[0],
                                    row: merge.split("-")[1],
                                    colCount: $(main).find("td[id="+merge+"]").prop('colspan'),
                                ...
                                if(spans.colCount > 1 || spans.rowCount > 1){
                                    records2[row][$(this).prop("id").split("-")[0]].spans = spans;
                                }
                                $(main).find("td[id="+merge+"]").each(function(){
                                    $(this).prop("rowspan", parseInt($(this).prop("rowspan"))-1)
                                });
                            ...
                            // 记录删除单元格的行内样式
                            if($(this).attr("style")){
                                records2[row][$(this).prop("id").split("-")[0]].lineStyle = $(this).attr("style");
                            }
                        });
                    ...
                    $.fn.jexcel.defaults[id].cellDataArray.splice(parseInt(rowNumber), numOfRows);
                    ...
                    if ($.fn.jexcel.ignoreEvents != true) {
                        if (typeof(options.ondeleterow) == 'function') {
                            options.ondeleterow($(this), rowNumber, numOfRows, records, records2);
                        }
                    }
                }
            } else {
    ...
    setHeight : function (row, height) {
        if (height > 0) {
            ...
            if (typeof(row) == 'object') {
                row = $(row).parent().prop('id').split('-');
                row = row[1];
            }

            var row = $(this).find('tbody .r' + row);
            if (row.length) {
                $(row).prop('height', height);
            }
        }
    },
    ...
    copyData : function(o, d) {
        ...
        var firstCell = $(this).find('.highlight:first').prop('id').split('-');
        ...
        for (j = py; j <= uy; j++) {
            ...
            for (i = px; i <= ux; i++) {
                ...
                // Update non-readonly
                if ($(cell).length && ! $(cell).hasClass('readonly hide')) {
                    var newValue = data[posy][posx];
                    //  下面是处理当遇到单元格合并时如何处理的代码
                    // var parentCell = $(this).find('#'+(parseInt(firstCell[0])+posx)+'-'+(parseInt(firstCell[1])-posy));
                    // if($(parentCell).hasClass('hide')){
                    //     var merge = $(parentCell).data('merge');
                    //     var newValue = $(this).jexcel('getValue', $(this).find('td[id='+merge+']'));
                    // }
                    // Keep cells history
                    records.push({
                        col: i,
                        ...
                        cell: $(cell),
                        newValue: newValue,
                        // 可以通过赋值cellData属性来复制样式及其绑定的信息等,里面暂时不包含单元格的合并信息
                        // cellData: clone($.fn.jexcel.defaults[$.fn.jexcel.current].cellDataArray[parseInt(firstCell[1])-posy][parseInt(firstCell[0])+posx]),
    ...
    executeFormula : function (cellId) {
        ...
        if (value === null || value == undefined || (''+value) == 'NaN' || (''+value) == 'Infinity') {
            ...
            // 增加公式计算时触发onchange事件函数
            if ($.fn.jexcel.ignoreEvents != true) {
                if (typeof($.fn.jexcel.defaults[id].onchange) == 'function') {
                    $.fn.jexcel.defaults[id].onchange(main, cell, value, $.fn.jexcel.defaults[id].values[letter]);
                }
            }
            // title显示计算公式
            // Create chain
            var regex = /([A-Z]+[0-9]+)*/g;
            var formula_data = formula;
            var elements = formula.match(regex).filter(function(n) { return n != '' });
            $.each(elements, function(i, v){
                formula_data = formula_data.replace(v, $.fn.jexcel.defaults[id].values[v]);
            });
            formula_data = formula_data.substr(1) + '=' + jsws(value);
            $(cell).attr('title', formula_data);
            ...
            // New cell value
            var cellDataArray = clone($.fn.jexcel.defaults[id].cellDataArray[cellId[1]][cellId[0]]);
            if (undefined !== cellDataArray.mask) {
                if (! $.fn.limsMask) {
                    console.error('Jexcel: it was not possible to load the mask plugin.');
                } else {
                    value = $(cell).limsMask(value, cellDataArray);
                }
            }
        ...
        if ($.fn.jexcel.defaults[id].historyIndex >= 0) {
            ...
            if (historyRecord.action) {
                ...
                // Create the deleted row back
                    ...
                    for (var row = rowNumber; row < rowNumber + numOfRows; row++) {
                        $(this).jexcel('insertRow', [historyRecord.action.rowData[row], historyRecord.action.rowData2[row]], row);
                    }
                ...
                // Create the deleted column back
                    ...
                    for (var col = columnNumber; col < columnNumber + numOfColumns; col++) {
                        $(this).jexcel('insertColumn',
                            [historyRecord.action.columnData[columnIndex], historyRecord.action.columnData2[columnIndex]], 
                            {
                ...
                }else if (historyRecord.action.type == 'mergeCell') {
                    // 拆分单元格
                    $(this).jexcel("unmergeCell", historyRecord.action.selection[0], historyRecord.action.selection[1]);
                    $(this).jexcel('updateSelection', historyRecord.action.selection[0], historyRecord.action.selection[1]);
                }else if (historyRecord.action.type == 'unmergeCell') {
                    // 合并单元格
                    $(this).jexcel("mergeCells", historyRecord.action.spans);
                    $(this).jexcel('updateSelection', historyRecord.action.selection[0], historyRecord.action.selection[1]);
                }else if(historyRecord.action.type == "setStyle"){
                    // 更改单元格样式
                    $(this).jexcel("setStyle", historyRecord.action.styles.oldStyle);
                    $(this).jexcel('updateSelection', historyRecord.action.selection[0], historyRecord.action.selection[1]);
                }
            } else {
                ...
                for (var i = 0; i < historyRecord.cellChanges.length; i++) {
                    ...
                    records.push({
                        ...
                        cellData: historyRecord.cellChanges[i].cellData
                    });
                }
        ...
        if ($.fn.jexcel.defaults[id].historyIndex < $.fn.jexcel.defaults[id].history.length - 1) {
            ...
            if (historyRecord.action) {
                ...
                // Move row
                    ...
                    // 合并单元格
                    $(this).jexcel("mergeCell", historyRecord.action.selection[0], historyRecord.action.selection[1]);
                    $(this).jexcel('updateSelection', historyRecord.action.selection[0], historyRecord.action.selection[1]);
                }else if (historyRecord.action.type == 'unmergeCell') {
                    // 拆分单元格
                    $(this).jexcel("unmergeCell", historyRecord.action.selection[0], historyRecord.action.selection[1]);
                    ...
                    // 更改单元格样式
                    $(this).jexcel("setStyle", historyRecord.action.styles.newStyle);
                    $(this).jexcel('updateSelection', historyRecord.action.selection[0], historyRecord.action.selection[1]);
                }
            } else {
                // Select cell
    ...
    createCell : function(i, j) {
        ...
        if (options.columns[i].type == 'hidden') {
            ...
            if (options.cellDataArray[j][i].readOnly == true) {
        ...
        // Readonly
        if (options.cellDataArray[j] && options.cellDataArray[j][i]) {
            // Readonly
            if(options.cellDataArray[j][i].readOnly){
                $(td).addClass('readonly', 'readonly');
            }
            // Add custom css class to column
            if (options.cellDataArray[j][i].cssClass) {
                $(td).addClass(options.cellDataArray[j][i].cssClass);
            }
        }
    ...
    updateTableReferences : function(referenceCol, referenceRow) {
                    ...
                    if (k1 == 0) {
                        ...
                        if (k1 >= referenceCol) {
                            ...
                            if (val.substr(0, 1) == '=') {
                                ...
                                if (coord[0] != k1 - 1) {
                                    // Update formulas
                                    val = $.fn.jexcel('shiftFormulaByColumn', val, k1 - 1 - coord[0], referenceCol);
                                ...
                                if (coord[1] != k) {
                                    // Update formulas
                                    val = $.fn.jexcel('shiftFormulaByRow', val, k - coord[1], referenceRow);
        ...
        // Find cols
        if (referenceCol > -1 && $.fn.jexcel.defaults[id].showColumnHeader) {
        ...
        // Update
        $(this).jexcel('updateAllCellsWithFormulas');
        $(this).jexcel('updateSpans');
        $(this).jexcel('mergeCells');
        $(this).jexcel('afterChange');
    },
     ...
     */
    shiftFormulaByColumn : function(value, index, referenceCol) {
        // Default
        if (! index) {
            index = 1;
        }
     ...
     * @param integer number of shifts
     */
    shiftFormulaByRow : function(value, index, referenceRow) {
        // Default
        if (! index) {
            index = 1;
        }
    ...
    destroy : function () {
        ...
        if (! Object.keys($.fn.jexcel.defaults).length) {
            ...
            $(window).off('resize', $.fn.jexcel.windowResize);
