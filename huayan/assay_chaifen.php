...
if($_GET['action']=='chaifen'){
	...
	while($row=$DB->fetch_assoc($R)){
		...
		$var[$row['id']] = $row['tid'];
		$lines.=$AssayApp->temp('hyd/assay_chaifen_line');
	}
	if($hyd_02>0){
	...
	echo $AssayApp->temp('hyd/assay_chaifen');
	...
	foreach ($hyd as $key => $value) {
		if(in_array($key,array('sign_date_01', 'sign_date_012', 'sign_date_02', 'sign_date_03', 'sign_date_04')) && empty($value)){
			$hyd_data[] = "`$key`=NULL";
		}else{
			$hyd_data[] = "`$key`='$value'";
		}
