...
class Jcrw_listApp extends LIMS_Base {
    ...
    */
    public function index(){
        ...
        $trade_global['daohang'][] = array('icon'=>'','html'=>'检测任务列表','href'=>$current_url);
        // 记录下本页面的导航到 session中
        $_SESSION['daohang']['ahlims']  = $trade_global['daohang'];
        $this->disp('xd_csrw/jcrw_list',get_defined_vars());
    }
    /**
     * 功能：
     * 作者：Mr Zhou
    ...
    */
    public function jcrw_list($fid=0){
        ...
        $sql_where = implode(' AND ', $sql_where_arr);
        //指定递增排序还是递减排序
        $order = !isset($_GET['sort']) ? 'DESC' : strtoupper(trim($_GET['order']));
        ...
        while ($row = $DB->fetch_assoc($query)) {
            ...
            if(empty($row['bar_code']['total'])){
                ...
                if( $row['status'] == '5' ){  //生成化验单前
                    if($u['xd_csrw']){
                        $operation[] = "<a href=\"{$rooturl}/xd_csrw/fp_csrw.php?cyd_id={$row['id']}\">分配测试任务</a>";
                    }
                }else{  //已生成化验单
                    $operation['fp_csrw'] = "<a href=\"{$rooturl}/xd_csrw/fp_csrw.php?cyd_id={$row['id']}\">修改测试任务</a>";
                    $operation['rwjs'] = "<a href=\"{$rooturl}/xd_csrw/rwjs.php?cyd_id={$row['id']}\" target=\"_blank\">任务接收单</a>";
                    $operation['yply'] = "<a href=\"{$rooturl}/xd_csrw/yply.php?cyd_id={$row['id']}\" target=\"_blank\">样品领用单</a>";
                    $operation['pay_list'] = "<a href=\"{$rooturl}/huayan/ahlims.php?app=pay_list&cyd_id={$row['id']}&year={$_GET['year']}&month={$_GET['month']}\">查看化验单</a>";
                    // $global['jcrw_list']配置检测任务列表要显示的按钮
                    if($global['jcrw_list']){
                        foreach($operation as $key => $value){
                            if(!in_array($key, $global['jcrw_list'])){
                                unset($operation[$key]);
                            }
                        }
                    }
    ...
    */
     ...
     * 功能描述：测试任务下达列表
    */
    public function csrwxd(){
        $u      = $this->_u;
        $DB     = $this->_db;
        $fzx_id = $this->fzx_id;
        global $global,$trade_global,$rooturl,$current_url;
        // 导航（前面部分在config.php中赋值）
        $trade_global['daohang'][] = array('icon'=>'','html'=>'测试任务下达','href'=>$current_url);
        // 记录下本页面的导航到 session中
        $_SESSION['daohang']['ahlims']  = $trade_global['daohang'];
        $this->disp('xd_csrw/csrwxd',get_defined_vars());
    }
    public function csrwxd_list(){
        $u      = $this->_u;
        $DB     = $this->_db;
        $fzx_id = $this->fzx_id;
        ...
        // `cyd_id`, `fzx_id`, `cyd_bh`, `site_type`, `sites`, `cy_flag`, `cy_user`, `cy_user2`, `cy_date`, `water_type`, `yp_count`, `status`, `cy_user_qz`, `cy_user_qz2`, `cy_user_qz_date`, `sh_user_qz`, `sh_user_qz_date`, `jmd_id`, `jy_user`, `ys_user`, `ys_date`, `ys_count`, `ys_result`, `ys_notes`, `csrw_xdcy_user`, `csrw_xdcs_user`, `xdcy_qz_date`, `xdcs_qz_date`, `cy_dept`, `jc_dept`, `jc_yiju`, `hyd_count`, `hyd_wc_count`, `jcwc_date`, `sj_wc_date`, `note`, `cy_note`, `create_user`, `create_date`, `modify_user`, `modify_time`, `csrw_tzd_note`, `qita_zk`, `group_name`, `save_flag`, `bar_code_position`, `xc_exam_value`, `xmfb`, `cy_rwxd_user`, `cy_rwxd_user_qz_date`, `cy_rwjs_user`, `cy_rwjs_user2`, `cy_rwjs_qz_date`, `rq_info`, `snkb`, `json`

        // 

        $sql = "SELECT `rec`.`id`, `cyd_id`, `sid`, `water_type`, `site_name`, `cy_time`, `cy_date`, `assay_values`, `snpx_item`, `jbhs_item`, `status`, `zk_flag`, `by_id`, `bar_code`
                    FROM `cy_rec` AS `rec`
                    WHERE 1 AND `cyd_id`='{$cyd_id}' ORDER BY `id` ASC";
        ...
        // echo json_encode(array('total'=>$total,'rows'=>$data_list));
        die(json_encode(array('error'=>'0','data'=>$data_list)));
    }
}