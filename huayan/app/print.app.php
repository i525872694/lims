...
class PrintApp extends LIMS_Base
{
    ...
    */
    ...
    {
        global $arow,$zhanming;
        $u = $this->_u;
        $DB=$this->_db;
        $fzx_id = $this->fzx_id;
        global $global,$trade_global,$rooturl,$rootdir,$current_url,$zong_biao,$heng_biao;
        ...
        foreach ($tids as $key => $tid) {
            ...
            if (is_file($filePath)) {
				...
				$arow['initText'] = file_get_contents($filePath);
                $plan_tpl = $this->temp('hyd/assay_form_hyd_new', get_defined_vars());
                // 提取extraxjs的内容
                ...
                $print_data[$arow['id']] = array(
                    ...
                    'lines_data'=> json_encode(get_assay_hyd_line($tid, $table_name, 3)),
                ...
                $plan_tpl = $this->temp('hyd/assay_form_hyd', get_defined_vars());
                // 提取extraxjs的内容
                preg_match("/<script.*extrajs.*>(.*)<\/script>/isU", $plan_tpl, $extrajs);
                $print_data[$arow['id']] = array(
                    'print_html'=> '',
                    ...
                    'extrajs'	=> $extrajs[1],
                    'line_tpl'	=> $line_tpl,
                    'lines_data'=> json_encode(get_assay_hyd_line($tid, $table_name, 1)),
                    'plan_tpl'	=> preg_replace('/<script.*>(.*)<\/script>/isU', '', $plan_tpl)
                );
            }
        }
    ...
    public function print_all()
    {
        ...
        $print_data = $hyd_pdf = array();
        $tids = [];
        $sql = "SELECT bm.* FROM `bt_muban` bm LEFT JOIN `xmfa` xf ON bm.id=xf.hyd_bg_id WHERE xf.act=1 GROUP BY hyd_bg_id";
        $query = $DB->query($sql);
        while ($row = $DB->fetch_assoc($query)) {
            $tids[] = $row;
        }
        foreach ($tids as $key => $arow) {
            $arow['vid']='174';
            // 表格纵横板式
            $zongheng   = $arow['zongheng'].'_biao';
            // 表格纵横板式的宽度
            ...
            if (is_file($filePath)) {
                ...
                eval('$hjtj_bt="'.gettemplate($this->file_path.'hjtj_bt').'";');
                // 获取化验单签字表单
                $assay_sign_form = get_assay_form_sign($arow)."<div style='text-align:right;'>{$arow['table_cname']}:{$arow['table_name']}</div>";
                // 其他数据
                @eval('$plan="'.gettemplate($this->file_path.'plan_'.$table_name).'";');
                @eval('$line_tpl="'.gettemplate($this->file_path.'line_'.$table_name).'";');
                $plan_tpl = $this->temp('hyd/assay_form_hyd', get_defined_vars());
                ...
                $print_data[$arow['id']] = array(
                    ...
                    'extrajs'   => $extrajs[1],
                    'line_tpl'  => $line_tpl,
                    'lines_data'=> json_encode([$line_tpl]),
                    'plan_tpl'  => preg_replace('/<script.*>(.*)<\/script>/isU', '', $plan_tpl)
                );
            }
        }
