...
class JcxmApp extends LIMS_Base {
	...
	*/
	...
	public function get_error_data(){
		...
		// 主辅测为同一人的保留主测,清空辅测
		$DB->query("UPDATE `xmfa` SET `userid2`='0' WHERE `fzx_id`='{$fzx_id}' AND `act`='1' AND `userid`=`userid2`");

		// 未设置检测方法，人员，表格，单位的
		$sql_1 = "SELECT `id` FROM `xmfa` WHERE `fzx_id`='{$fzx_id}' AND `act`='1' AND ( `userid`=`userid2` OR `fangfa`='0' OR `hyd_bg_id`='0' OR `unit`='' )";
		$query_1 = $DB->query($sql_1);
		while ( $row = $DB->fetch_assoc($query_1) ) {
			$error_data[] = intval($row['id']);
		}
		// 未设置默认的,存在多个默认方法的自动设置或保留一个
		...
		while ( $row = $DB->fetch_assoc($query_2) ) {
			...
			if( 1 != intval($DB->num_rows($query_3)) ){
				// $error_data[] = $row['id'];
				// 其他方法设置为非默认
				$DB->query("UPDATE `xmfa` SET `mr`='0' WHERE `fzx_id`='{$fzx_id}' AND `lxid`='{$row['lxid']}' AND `xmid` ='{$row['xmid']}'");
				// 将第一个方法设置为默认
				$row_3 = $DB->fetch_one_assoc("SELECT `id` FROM `xmfa` WHERE `fzx_id`='{$fzx_id}' AND `lxid`='{$row['lxid']}' AND `xmid` ='{$row['xmid']}'");
				$DB->query("UPDATE `xmfa` SET `mr`='1' WHERE `fzx_id`='{$fzx_id}' AND `id`='{$row_3['id']}'");
			}
		}
	...
	*/
	public function xmfa_list($fid=0){
		...
		$sql_select	= "SELECT `x`.*, `table_cname`, `lx`.`lname`,
				...
				`yq`.`yq_mingcheng` AS `td4`, `yq`.`yq_xinghao` AS `td5`, `yq`.`{$yq_bh}` AS `yq_bh`,
		...
		if( intval($fid) ){
			...
			if( '4' == $_GET['show_type']){
				...
				//检测项目
				if( intval($_GET['vid']) ){
					$sql_where_arr[] = "AND `av`.`id`='{$_GET['vid']}' ";
				}
			}
			//组合WHERE条件
			...
			$search_str_arr = array('value_C','method_name','method_number','yq_mingcheng','yq_xinghao','yq_chucangbh','table_cname');
			if( !empty($search_str) ){
				// 关键字搜索
				###--是否认证--###
				if( in_array($search_str, array('已认证','未认证'))){
					$sql_where_arr[] = "AND `zzrz` " . ( '已认证' == $search_str ? '=' : '!=' ) . " '1'";
				}else{
			...
			//指定排序列
			$order_by_value_C = "`av`.`seq` ASC, CONVERT(`av`.`value_C` USING gbk) ASC, `lxid` ASC ,";
			...
			switch ($sort) {
				...
				case 'value_C':
					$order_by_value_C = '';
					$sort = "`av`.`seq` ASC, CONVERT(`av`.`value_C` USING gbk) {$order}, `lxid` ASC";
					break;
				case 'fangfa':
					$sort = "`am`.`method_number` {$order},`am`.`method_name` {$order}";
					break;
				case 'yiqi':
					$sort = "`yq`.`yq_mingcheng` {$order},`yq`.`yq_xinghao` {$order},`yq`.`{$yq_bh}` {$order}";
					break;
				default:
					$sort = "`{$sort}` {$order}";
					break;
		...
		while ($row = $DB->fetch_assoc($query)) {
			...
			if( !empty($search_str) ){
				foreach ($search_str_arr as $value) {
					$row[$value] = preg_replace("/".$search_str."/", '<b style="color:#F00;">'.$search_str.'</b>', $row[$value],1);
				}
			}
			// 赋值
			$xmfa[] = $row;
	...
	*/
	public function xmfa_save(){
		...
		//允许修改的字段
		$columns = array('act','fangfa','xmid','jcx','unit','yiqi','userid','userid2','w1', 'w2','w3','w4','w5','hyd_bg_id','zzrz','mr','check_jcx','round_func');
		//有控制信息的条件
		...
		if( isset($_GET['is_blws']) && intval($_GET['is_blws']) ){
			...
			foreach ($_GET['blws'] as $key => $value) {
				$blws = trim($_GET['blws'][$key]);
				$yxsz = trim($_GET['yxsz'][$key]);
				$zdws = trim($_GET['zdws'][$key]);
	...
	*/
	private function get_all_leixing(){
		$DB		= $this->_db;
		$leixing= array();
		$query = $DB->query("SELECT `id`,`lname` FROM `leixing` WHERE `parent_id`='0' AND `act`='1' order by `sort` desc");
		while($lx = $DB->fetch_assoc($query)){
			$leixing[$lx['id']] = $lx['lname'];
			$sql_leixing = $DB->query("SELECT `id`,`lname` FROM `leixing` WHERE `parent_id`='{$lx['id']}' AND `act`='1' order by `sort` desc");
			while($xlx = $DB->fetch_assoc($sql_leixing)){
	...
	// 重置项目排列顺序
	public function init_order_xm(){
		$DB		= $this->_db;
		$fzx_id	= $this->fzx_id;
		// 修改或添加seq字段
		$columns = $this->get_columns('assay_value');
		if(!in_array('seq', $columns)){
			$sql = "ALTER TABLE `assay_value` ADD `seq` INT NOT NULL DEFAULT '999' COMMENT '排序id'";
            $query = $DB->query($sql);
		}
		// 重置排序
		$DB->query("UPDATE `assay_value` SET `seq` = '9999'");
		//获取默认排序
		$xm_px = $this->get_default_value_order(1);
		foreach($xm_px as $key => $vid){
			$DB->query("UPDATE `assay_value` SET `seq`='{$key}' WHERE `id`='{$vid}'");
		}
		return true;
	}
	/**
	 ...
	 * 功能描述：检测项目排序
	*/
	public function order_jcxm(){
		$DB		= $this->_db;
		$fzx_id	= $this->fzx_id;
		// 项目排序
		$jcxm = [];
		$sql = "SELECT `id`,`value_C`,`seq` FROM `assay_value` WHERE 1 ORDER BY `seq` ASC, CONVERT(`value_C` USING gbk)";
		$query = $DB->query($sql);
		while ($row = $DB->fetch_assoc($query)) {
			$jcxm[] = $row;
		}
		echo $this->temp('hyd/jcxm/order_jcxm', get_defined_vars());
	}
	/**
	 * 功能：
	 * 作者：Mr Zhou
	 ...
	 * 功能描述：检测项目排序保存
	*/
	public function order_jcxm_save(){
		$DB		= $this->_db;
		$fzx_id	= $this->fzx_id;
		$SQL = [];
		$order_values = $_POST['order_values'];
		if(empty($order_values)){
			die(json_encode(['error'=>0]));
		}
		foreach($order_values as $vid => $i){
			$DB->query("UPDATE `assay_value` SET `seq`='{$i}' WHERE `id`='{$vid}'");
		}
		die(json_encode(['error'=>0]));
	}
	/**
	...
	*/
	public function get_all_yiqi(){
		...
		$yq_bh = $global['hyd']['yq_bh'];
		empty($yq_bh) && ($yq_bh='yq_chucangbh');
		$sql = "SELECT `id`,`yq_mingcheng`,`yq_xinghao`,`{$yq_bh}` AS yq_bh FROM `yiqi` WHERE `fzx_id`='$fzx_id' ORDER BY CONVERT(`yq_mingcheng` USING gbk)";
		$yiqi = array( 0 => '--请选择仪器--' );
		$query = $DB->query($sql);
		while ($row=$DB->fetch_assoc($query)) {
			$yiqi[$row['id']] = $row['yq_mingcheng'].$row['yq_xinghao'].'['.$row['yq_bh'].']';
		}
		return $yiqi;
	}
	/**
