...
class BiaodingApp extends LIMS_Base {
		  ...
		  <form action="'.$rooturl.'/huayan/qx_data.php" method="get" name="form_select" class="modal-content">
			...
			<div class="modal-body">
			  ...
			  <table  class="table table-bordered center" style="width:100%" align=center>
				<tr><td>记录号</td><td>标准溶液名称</td><td>浓度</td><td>配制日期</td><td>标定日期</td><td>标定人</td></tr>
				'.$bd_lines.'
			  </table>
			</div>
	...
	*/
	public function related_to_hyd(){
		...
		if(intval($sc_bd[0])){
			$query = $DB->query("UPDATE `assay_pay` SET `bdid`='{$sc_bd[0]}',`scid`=0,`pzid`=0,`CA`='{$sc_bd[1]}',`CB`='{$sc_bd[2]}' WHERE `id`='{$hyd_id}' AND `fp_id`='{$fzx_id}'");
		}
		if($query){
