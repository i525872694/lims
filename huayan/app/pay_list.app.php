...
class Pay_listApp extends LIMS_Base {
	...
	*/
	public function pay_list($fid=0){
		...
		if( isset($_GET['year']) && intval($_GET['month']) && intval($_GET['day'])){
			$sql_where_arr[] = "DAY(cy.`cy_date`) = '{$_GET['day']}'";
		}
		// 分析人员
		...
		//指定排序列
		$sort = (isset($_GET['sort']) && !empty($_GET['sort']) ) ? trim($_GET['sort']) : '';
		switch ($sort) {
			case 'userid':
			...
			case 'assay_element':
				...
				break;
			default:
				$sort = "`cy`.`jcwc_date` ASC, `cy`.`cy_date` ASC";
				break;
		}
	...
	*/
	public function piliang_hb(){
		...
		if(!empty($_POST['ids'])){
			...
			$sql = "SELECT `ap`.`id`,`cyd_id`,`vid`,`fid`,`assay_element`,`cy`.`cy_date` FROM `assay_pay` `ap` LEFT JOIN `cy` ON `ap`.`cyd_id` = `cy`.`id` WHERE `ap`.`id` IN('{$ids}') AND (`sign_01`='' OR `sign_01` IS NULL ) {$check_users} AND `fp_id`='{$fzx_id}' ORDER BY `fid` ASC,`ap`.`id` ASC";
			$query = $DB->query($sql);
			while ($row = $DB->fetch_assoc($query)) {
				$assay_pays[$row['id']] = $row;
				$key = $row['fid'];
				if($global['hyd']['pay_hebing_key'] == 1){
					$key .= $row['cy_date'];
				}
				$assay_tids[$key][] = $row['id'];
			}
			foreach ($assay_tids as $fid => $row) {
				...
				$DB->query("SET @r:=-1");
				foreach ($row as $tid) {
					$DB->query("UPDATE `assay_order` SET `order_id`=(@r:=@r+1) WHERE `tid`='{$tid}' ORDER BY `order_id` ASC");
				}
				// 更新order表tid，删除多余pay表数据
				$tids = implode("','", $row);
		...
		if(!empty($_POST['s'])){
			...
			foreach($_POST['s'] as $tid){
				...
				if(!empty($load_rs[$column_name['COLUMN_NAME']])){
					$_GET['tid'] = $rs['id'];
					$_GET['vid'] = $rs['vid'];
					$_GET['fid'] = $load_rs[$column_name['COLUMN_NAME']];
					include(__ROOTDIR__."/autoload/loadtable.php");
					if( $count > 0 ){
						$zr_data['count']  += $count;
						$zr_data['tids'][] = $tid;
