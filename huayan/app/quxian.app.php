...
class QuxianApp extends LIMS_Base {
	...
	*/
	public function index($sc_form=''){
		...
		$arow	= $this->get_sc_data($sc_id,'type,vid,uid',true);
	...
	*/
	public function sc_list(){
		...
		//$DB->query("UPDATE `standard_curve` SET `status`='已停用' WHERE `fzx_id`='$fzx_id' AND `status`!='已停用' AND `start_time`<'".date('Y-m-d',strtotime('-90 days'))."'");
	...
	*/
	protected function get_xm_list($sc_type,$vid=0){
		...
		if('1'==$sc_type){
			...
			// 因为标液的有效日期格式可能为年月，例如2019-03，这种数据在mysql查询比较时不能按照日期格式进行比较，所以判断此格式时末尾拼接-31，表示当月也有效
			$query_bd = $DB->query("SELECT DISTINCT `vid` FROM `bzwz_detail` AS bd LEFT JOIN `bzwz` ON `bzwz`.id=bd.`wz_id` WHERE `bzwz`.`wz_type`='标准溶液' AND `bzwz`.fzx_id='{$fzx_id}' AND IF(`bzwz`.`time_limit`!='' AND `bzwz`.`time_limit` IS NOT NULL AND YEAR(`bzwz`.`time_limit`) IS NULL , CONCAT(`bzwz`.`time_limit`, '-31'), `bzwz`.`time_limit`) > CURRENT_DATE()");
	...
	*/
	public function view_sc($arow=null){
		...
		// 曲线计算配置信息
		$Zk_quxianApp = new Zk_quxianApp();
		$arow['config'] = $Zk_quxianApp->get_quxian_config($arow['vid']);
		$content = call_user_func_array([$this, 'viewScType'.$arow['type']], [$arow]);
	...
	*/
	protected function viewScType1($arow){
		...
		//获取曲线方程式
		$arow['quxian'] = self::get_sc_gongshi($arow);
	...
	*/
	public function sel_sc(){
		...
		$sc_lines = '';
		//  根据化验单的生成日期来检索之前半年内的曲线，而不再根据当前时间来检索
		$hydRow = $DB->fetch_one_assoc("SELECT if(`start_time`!='',`start_time`,`create_date`) `create_date` FROM `assay_pay` WHERE `id`='{$hyd_id}'");
		$end_date = date('Y-m-d', strtotime("{$hydRow['create_date']} +10 days"));
		$str_date = date('Y-m-01', strtotime("{$hydRow['create_date']} -6 month"));
		$query = $DB->query("SELECT `id`,`CA`,`CB`,`CR`,`uid`,`userid` AS `create_man`,`td31` AS `create_date`,`status`,`sign_01` FROM `standard_curve` WHERE `fzx_id`='{$fzx_id}' AND `vid`='{$vid}' AND `td31` BETWEEN '{$str_date}' AND '{$end_date}' ORDER BY `td31` DESC,`id` DESC");
		while($row=$DB->fetch_assoc($query)){
			//是否具有修改权限
			$row['canModi'] = $this->canModi($row);
			//操作按钮，查看修改，复制，删除
			...
			//获取曲线方程式index
			$row['quxian'] = self::get_sc_gongshi($row);
			empty($row['CA']) && ($row['CA']='/');
			empty($row['CB']) && ($row['CB']='/');
			empty($row['CR']) && ($row['CR']='/');
	...
	*/
	public function related_to_hyd(){
		...
		if(intval($sc_bd)){
			$arow = $this->get_sc_data($sc_bd[0], '`CA`,`CB`,`CT`,`CR`');
			$query = $DB->query("UPDATE `assay_pay` SET `scid`='{$arow['id']}',`bdid`=0,`pzid`=0,`CA`='{$arow['CA']}',`CB`='{$arow['CB']}',`CT`='{$arow['CT']}',`CR`='{$arow['CR']}' WHERE `id`='{$hyd_id}' AND `fp_id`='{$fzx_id}'");
		}
		if($query){
	...
	*/
	protected function get_sc_data($sc_id,$column='*',$can_null=false){
		...
		// 解析json数据
		if($arow['json']){
			$arow['json'] = json_decode($arow['json'], true);
		}
		if($arow['CA']){
			$arow['CAA'] = ($arow['CA'] >= 0) ? '+'.$arow['CA'] : $arow['CA'];
		}
		return $arow;
	}
	/**
	...
	*/
	static function get_sc_gongshi($data){
		global $global;
		if( '' === $data['CA'] || '' === $data['CB'] ){
			return '';
		}
		// 给大于等于0的截距添加加号(+)
		$data['CAA'] = (float)$data['CA'] >= 0 ? '+'.$data['CA'] : $data['CA'];
		// y=c²x+bx+a
		$data['CX'] = empty($data['CC']) ? '' : $data['CC'].$x.'²+';
		// 获取曲线公式显示格式
		$sc_gongshi = !empty($global['hyd']['sc_gongshi']) ?
				$global['hyd']['sc_gongshi'] : 'y={$data[CX]}{$data[CB]}x{$data[CAA]}';
		eval("\$quxian = \"{$sc_gongshi}\";");
		return $quxian;
	}
	/**
	 * 功能：获取曲线单位列表
	...
	*/
	public function dataSuYuan($a='',$b='',$c='',$d='',$e=''){
		...
		if(empty($arow['json']['退回'])){
			$xiuGaiLiYou = '';
		}else{
			$huiTuiLiYou = end($arow['json']['退回']);
	...
	*/
	public function getBzryBox(){
		...
		$Zk_quxianApp = new Zk_quxianApp();
		$muban_list = $Zk_quxianApp->get_sc_muban();
	...
	*/
	public function getBzryList(){
		...
		die(
			json_encode(
				[
					'sc_muban' => $Zk_quxianApp->get_quxian_config($vid)['sc_muban'],
					'temp' => call_user_func(array($this, 'getBzry'.($wz_type ? $wz_type : 1)), $vid)
				]
			)
		);
	}
	...
	*/
	private function getBzry1($vid){
		...
		//标准溶液
		// 因为标液的有效日期格式可能为年月，例如2019-03，这种数据在mysql查询比较时不能按照日期格式进行比较，所以判断此格式时末尾拼接-31，表示当月也有效
		$sql = "SELECT `bzwz`.`wz_bh`,`bzwz`.`wz_name`,`bzwz`.`amount`,`bzwz`.`amount`,`bzwz`.`unit`,`bzwz`.`time_limit`,bd.`consistence`,bd.`id` FROM `bzwz_detail` AS bd LEFT JOIN `bzwz` ON `bzwz`.`id`=bd.`wz_id` WHERE `bzwz`.fzx_id='$fzx_id' AND bd.`vid`='$vid' AND `bzwz`.`wz_type`='标准溶液' AND IF(`bzwz`.`time_limit`!='' AND `bzwz`.`time_limit` IS NOT NULL AND YEAR(`bzwz`.`time_limit`) IS NULL , CONCAT(`bzwz`.`time_limit`, '-31'), `bzwz`.`time_limit`) >= CURRENT_DATE() ORDER BY `bzwz`.`wz_bh`, `bzwz`.`time_limit`";
		$query = $DB->query($sql);
		$bzry_lines = '';
		while($row=$DB->fetch_assoc($query)){
			// 库存为0时也允许 添加
	...
	*/
	public function modi_sc(){
		...
		$arow = array();
		if($sc_id){
			$arow = $this->get_sc_data($sc_id,'uid,vid,json');
		...
		$key_val_arr[]  = "`json`='".JSON(array_merge($arow, $_POST['json']))."'";
		$key_val_arr[]	= "`assay_element`='".JSON_addslashes($_SESSION['assayvalueC'][$_POST['vid']])."'";
		//这是保存并签字
		...
		if($sc_type==2){
			...
			// 曲线计算配置信息
			$Zk_quxianApp = new Zk_quxianApp();
			$config = $Zk_quxianApp->get_quxian_config($arow['vid']);
			//曲线数据计算
			if('y' != $config['use_xy']){
			...
			if(is_array($data)){
				// 获取Ｒ值修约保留位数
				$round_r_ws = $config['round_r']['blws'][0][1];
				// 只舍不进
				if($config['round_r']['round_function'] == 'floor'){
					// R值修约只舍不入，通过10的幂次方运算来操作
					$round_r_xs = pow(10, $round_r_ws);
					// 先乘以系数舍去小数取整后再还原小数位数
					$jg['R'] = floor($data['R']*$round_r_xs)/$round_r_xs;
				}else if(!function_exists($config['round_r']['round_function'])){
					// 默认使用_round修约
					$jg['R'] = _round($data['R'], $round_r_ws);
				}else{
					$jg['R'] = call_user_func($config['round_r']['round_function'], $data['R'], $round_r_ws);
				}
				// 相关系数R的最大值
				if(is_numeric($config['round_r']['max_num']) && (float)$jg['R'] >= (float)$config['round_r']['max_num']){
					$jg['R'] = $config['round_r']['max_num'];
				}
				// 截距，斜率
				$jg['A'] = auto_round($data['a'], $config['round_a']);
				$jg['B'] = auto_round($data['b'], $config['round_b']);
				// ｔ值检验
				$jg['T'] = auto_round($data['T'], $config['round_t']);
				$CA	= trim($jg['A']);
				$CB	= trim($jg['B']);
				$CC	= trim($jg['C']);
				$CR	= trim($jg['R']);
				...
				if($config['check_t']){
					...
					$remarks = $jg['t'].trim(preg_replace("/(t=.{0,};t\(0.05\)=.+[截距合格]{1})/",'',$_POST['td30']));
		...
		$equation = self::get_sc_gongshi(array( 'CA'=>$CA,'CB'=>$CB,'CC'=>$CC ));
		...
		echo json_encode(array('sc_id'=>$sc_id,'error'=>'0','content'=>$content,'data'=>$data));
	}
	/**
	 * 功能：曲线计算
	 * 作者：Mr Zhou
	...
	*/
	private function standard_curve($x,$y){
		...
		// T值数据对照表,曲线T值检验,分为双边和单边,默认单边
		if('shuang_bian' == $global['hyd']['sc_check_CT']){
			$_data=array(0,12.706,4.303,3.182,2.776,2.571,2.447,2.365,2.306,2.262,2.228,2.201,2.179,2.160,2.145);
		}else{
	...
	*/
	 ...
	 * 功能描述：
	*/
	private function set_sc_json_data($arow, $key, $data){
		// 存在曲线表standard_curve的json字段中
		if(is_numeric($arow)){
			$arow = $this->get_sc_data($arow,'json',true);
		}
