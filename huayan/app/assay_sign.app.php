...
class Assay_signApp extends LIMS_Base {
	...
	public function assay_sign(){
		...
		switch ($sign_type) {
			case 'fx_qz':
				...
				}else if( $admin || in_array($u['userid'], array($arow['userid'], $arow['userid2']) ) || in_array($u['id'], array($arow['uid'], $arow['uid2']) ) ){
		...
		if( !intval($dhy_arr[$arow['vid']]) || 'assay_pay' != $table_name ){
			...
			$ztable_name = $DB->fetch_one_assoc("SELECT `table_name` FROM `bt_muban` WHERE `id`='{$arow['table_id']}';");
			//判断当前项目化验单模板是否为多合一模板
			if(in_array(reset($ztable_name),$dhy_arr['table_name'])){
				//获取多合一化验单列表
				$id_arr = array();
				$vid_str = implode(',',$dhy_arr['xm'][$dhy_arr[$arow['vid']]]);
				$query = $DB->query("SELECT `id`,`table_id` FROM `assay_pay` WHERE `cyd_id` = {$arow['cyd_id']} AND `vid` IN ({$vid_str})");
				while ( $row = $DB->fetch_assoc($query) ) {
					$ftable_name = $DB->fetch_one_assoc("SELECT `table_name` FROM `bt_muban` WHERE `id`='{$row['table_id']}';");
					//如果分项目和当前主项目使用了同一个多合一化验单模板
					if($ftable_name == $ztable_name){
						$id_arr[] = $row['id'];
					}
				}
	...
	*/
	public function assay_return_back(){
		...
		//将化验单退回到已开始状态
		$hyd_query = $DB->query("UPDATE `{$table_name}` SET {$clear_sign_name_sql} {$clear_sign_date_sql} `{$status}`='已开始',`json`='{$jsonStr}' WHERE `id` IN ({$id_str})");
