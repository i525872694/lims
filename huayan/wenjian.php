...
if($_POST['submit']){
	...
	{//遍历文件得到键名键值,value为文件名称
		$pdf=array_pop(explode('.',$value));
		$name_suffix= substr($value,0,strrpos($value,"."));
		if($pdf=='pdf' || $pdf=='xls'|| $pdf=='xlsx')
		{
			if($_FILES['upfile']['size'][0]<1024*1024*3)
			{
				...
				//判断文件夹是否存在,不存在就建立
				if(!is_dir("/home/files/".date(Ym)."")){
					mkdir("/home/files/".date(Ym)."",0777);
					chmod("/home/files/".date(Ym)."",0777);//一定要有不然新建的文件夹是755的权限
				}
