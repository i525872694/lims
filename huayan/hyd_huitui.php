...
if($_POST['action']=='hyd' && !empty($_POST['yuanYin'])&&!empty($_POST['id'])){//化验单退回
	...
	//以下是对json中 特殊符号的转义“\”和“"”需要进行两次转义，“'”需要进行一次转义
        ...
        $yxrow  = $DB->affected_rows();
        die(json_encode(($yxrow==1) ? array('error'=>'0','content'=>'') : array('error'=>'1','content'=>'签字失败，请刷新页面重试！')));
}else if($_POST['action']=='bzpz' && !empty($_POST['yuanYin'])&&!empty($_POST['id'])){//标准溶液配置记录退回
	$rsCyd	= $DB->fetch_one_assoc("SELECT `json` FROM `jzry` WHERE id='".$_POST['id']."'");
	if($rsCyd['json']!=''){
		$json	= json_decode($rsCyd['json'],true);
	}else{
		$json	= array();
	...
	$jsonStr= JSON($json);
	$DB->query("UPDATE `jzry` SET `sign_01`='',`sign_02`='',`sign_03`='',`json`='".$jsonStr."' WHERE id='".$_POST['id']."'");
	$yxrow	= $DB->affected_rows();
	die(json_encode(($yxrow==1) ? array('error'=>'0','content'=>'') : array('error'=>'1','content'=>'回退失败，请刷新页面重试！')));
}
...
if($_GET['action'] == 'cyd'){
	...
	$button_value	= '确认修改';
}else if($_GET['action'] == 'bzpz'){
	$placeholder	= '请在此处填写退回此标准配置记录表的原因';
	$button_value	= '确认退回';
}
