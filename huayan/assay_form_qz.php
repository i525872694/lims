...
if(!in_array(trim($_GET['qz']), array('jh','fh','sh'))){
	$PublicApp->reto('请求错误！',  $url[1], 'danger', 3);
}
//导航
...
$tid = intval($_GET['tid']);
if(!$tid){
	$PublicApp->reto('未提供正确化验单编号!',  $url[1], 'danger', 3);
}
...
foreach ($hyd_all as $hydRow => $hydRow_hyds) {
	...
	foreach ($hydRow_hyds as $key => $tid) {
		...
		$items .= $PublicApp->temp('/hyd/assay_form_qz_items');
	...
	$assay_form_hyds .= $PublicApp->temp('/hyd/assay_form_qz_row');
...
$PublicApp->disp('hyd/assay_form_qz');
?>