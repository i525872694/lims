...
include_once AH_PATH.'lib/make_bar_code.class.php';
/**
 * 函数名：get_zk_bar
 * 功能：获得质控样品编号
 * 作者：Mr Zhou
...
// 更改assay_order表数据排列顺序order_id
function change_order_id($sample, $new_id){
	global $DB,$global;
	// 更改顺序
	$sql = "UPDATE `assay_order` SET `order_id`=`order_id`+1 WHERE `tid`='{$sample['tid']}' AND `order_id`>{$sample['order_id']}";
	$DB->query($sql);
	$sql = "UPDATE `assay_order` SET `order_id`={$sample['order_id']}+1 WHERE `id`='{$new_id}'";
	$DB->query($sql);
	$DB->query("SET @r:=-1");
    $DB->query("UPDATE `assay_order` AS `ao` SET `order_id`=(@r:=@r+1) WHERE `tid`='{$sample['tid']}' ORDER BY `order_id` ASC");
}
function zhikong($sample){
	...
	switch ($_GET['action'])
	{
	  case  '2'://添加空白
	  {
		$kongbai = $makeBarCode->get_new_kongbai($sample);
		if($kongbai['error']){
			$content = $kongbai['content'];
		}else{
			$sql = "INSERT INTO `assay_order` SET `cyd_id`='{$sample['cyd_id']}',`vid`='{$sample['vid']}',`cid`='{$sample['cid']}',`mid`='{$sample['mid']}',`tid`='{$sample['tid']}',`sid`='{$kongbai['sid']}',`site_name`='室内空白',`hy_flag`='-2',`bar_code`='{$kongbai['bar_code']}',`bar_code_position`='{$sample['bar_code_position']}',`assay_over`='{$sample['assay_over']}',`vd28`='{$_GET['vd28']}'";
			$result  = $DB->query($sql);
			$new_id = $DB->insert_id();
			change_order_id($sample, $new_id);
	  ...
	  {
		$zk_data = JSON($_GET);
		$sql	 = "UPDATE `assay_order` SET `zk_data`='{$zk_data}',`ping_jun`='',`xiang_dui_pian_cha`='',`ping_jia`='' WHERE id='{$_GET['id']}'";
		$result	 = $DB->query($sql);
	  ...
	  case '4'://添加单点标液
	  {
		...
		// !empty($_GET['barCode']) && $zk_set['zky_name'] = $_GET['barCode'];
	  	...
	  	for ($i=0; $i < $addZkyGs; $i++) {
	  		$zikongyang = $makeBarCode->get_new_zikongyang($sample);
	  		$sql	= "INSERT INTO `assay_order` SET `cyd_id`='{$sample['cyd_id']}',`vid`='{$sample['vid']}',`cid`='{$sample['cid']}',`mid`='{$sample['mid']}',`tid`='{$sample['tid']}',`sid`='{$sample['sid']}',`hy_flag`='-4',`bar_code`='{$zikongyang['bar_code']}',`site_name`='$bar',`bar_code_position`='{$sample['bar_code_position']}',`assay_over`='{$sample['assay_over']}',`vd28`='{$_GET['piHao']}',`vd29`='{$_GET['biaoZhunZhi']}',`vd30`='{$_GET['buQueDingDu']}',`vd31`='{$_GET['vd31']}',`vd32`='{$_GET['vd32']}'";
	  		$result	 = $DB->query($sql);
	  		$new_id = $DB->insert_id();
			change_order_id($sample, $new_id);
	  	}
	  ...
	  {
		$bar_code = '';
		!empty($_GET['barCode']) && $bar_code = ",`bar_code`='{$_GET['barCode']}',`site_name`='{$_GET['barCode']}'";
	  $sql	= "UPDATE `assay_order` SET `vd28`='{$_GET['piHao']}',`vd29`='{$_GET['biaoZhunZhi']}',`vd30`='{$_GET['buQueDingDu']}',`vd31`='{$_GET['vd31']}',`vd32`='{$_GET['vd32']}',`ping_jun`='',`xiang_dui_pian_cha`='',`ping_jia`='' {$bar_code}";
	  	if( '1' != $_GET['add_all']){
