...
if(!in_array(trim($_GET['qz']),array('jh','fh','sh'))){
	die(json_encode(['error'=>'1', 'content'=>'请求错误！']));
}
...
foreach ($hyd_all as $hydRow => $hydRow_hyds) {
	...
	foreach ($hydRow_hyds as $key => $tid) {
		...
		$assay_form_hyds[$hydRow][$tid]['items'] = $AssayApp->temp('/hyd/assay_form_qz_items');
	...
	$assay_form_hyds[$hydRow]['hydRow']		= $AssayApp->temp('/hyd/assay_form_qz_row');
