...
while ($hymds= $DB->fetch_assoc($hymd)) {
  ...
  $filename1 = "{$pdf_files}shuyuan/{$hydev}/{$hyfile}/{$hymds['md5']}.gz";
    if (!file_exists($filename1)) {
        die(PublicApp::show_message('溯源目录01不存在,请联系管理员检查.', 'danger'));
    }
    //解压缩
    exec("gzip -d $filename1", $out2, $status2);
    ...
    $filename2 = "{$pdf_files}shuyuan/{$hydev}/{$hyfile}/{$hymds['md5']}";
    if (!file_exists($filename2)) {
        die(PublicApp::show_message('溯源目录02不存在,请联系管理员检查.', 'danger'));
    }
    exec("gzip {$filename2}", $out2, $status2);
}
$header = '原始记录表修改记录';
$syHtml = preg_replace('/<script.*>(.*)<\/script>/isU', '', $syHtml);
// 声明化验单对象
$AssayApp = new AssayApp();
echo $AssayApp->temp('hyd/dataSuYuan', get_defined_vars());