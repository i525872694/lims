...
if(isset($_SESSION['u']['userid'])){
	...
	if( $_POST['token_key'] == $_SESSION['token_key']['login'] ){
		$where_fzx = '';
		if(isset($xianzhi_fzx_login) && $xianzhi_fzx_login === true && intval($current_fzx_id)){
			$where_fzx = "AND `fzx_id`='{$current_fzx_id}'";
		}
		$u = $DB->fetch_one_assoc( "SELECT u.*,h.hub_name,h.is_zz FROM `users` u LEFT JOIN `hub_info` h ON u.fzx_id=h.id WHERE `nickname` = '{$nickname}' AND `password` = '{$password}' {$where_fzx} LIMIT 1" );
		...
		//判断用户是否已经登录 
		/*$login=$DB->fetch_one_assoc("select `is_login` from `users` where `nickname` = '{$nickname}' AND `password` = '{$password}'");
		if($login['is_login']=='1'&&$socket_status){
			echo "<script src='{$rooturl}/js/jquery.js'></script>";
			echo "<script>
					if(confirm('您的账号已登录，是否强制下线其他设备?')){
						$.post('exit.php',{uname:'$nickname'},function(data){
		...
		if(isset($_REQUEST['ajax']) && '1' == $_REQUEST['ajax']){
			...
			$return = curl_exec ( $ch );
			curl_close ( $ch );
			$user_arr=json_decode($return,JSON_FORCE_OBJECT);
			//判断用户是否登录
			...
			//判断密码是否是lims123,是的话,弹出修改密码界面
			if(trim($_POST['password']) == 'lims123') gotourl($rooturl.'?modi_pwd=1');
			gotourl($rooturl);
			toexit();
		}
	}
...
<body>
	<div class="title">
		<!--<img src="img/lims.jpg" alt="" >-->
		<div>
			<div class="login_two" style="width:850px;margin:0 auto;">
				<img src="./img/lims.jpg" style="width:120px;text-align:right;">
				云南省水文水资源局实验室信息管理系统
				<p>Laboratory Information Management System</p>
				<br />
				<?php echo $current_fzx_name;?>
			</div>
		</div>
	</div>
	<div class="box_login">
