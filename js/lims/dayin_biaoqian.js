...
function fa(){
 ...
 // if(!newbar.match(/[A-Z]{2}\d{6}-\d{4}/)){
  if(!newbar.match(/[0-9]{2}[A-Z]{2}\d{6}\d{3}$/)){
      alert("样品编号格式有误！");
      return false;
  }
...
function xiucode(code,i){
        liu = code.substring(code.length-3);
        ibar = Number(liu)+Number(i);
        //console.log(String(ibar).length);
        aa = 3-String(ibar).length;
        if(aa>0){
            for(j=1;j<=aa;j++){
                ibar = "0"+String(ibar);
            }
        }
        code = code.substring(0,code.length-3)+ibar;
        return code;
}
function yzcode(va){
  var code = va.value;
  //if(!code.match(/^[A-Z]{2}\d{6}-\d{4}$/)){
     if(!code.match(/[0-9]{2}[A-Z]{2}\d{6}\d{3}$/)){
      $(va).css('color','red');
