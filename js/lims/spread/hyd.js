 ...
 *enter_next_input
 *enter_sheet_input
 *Copyright (c) 2017 Mr zhou (zhouguangli@anheng.com.cn)
*/
"use strict";
...
(function($) {
		...
		*/
		assay_form_init : function(asF){
			...
			asF.find(".fa_change_"+asF.id).change(function(){
				location.href = trade_global.rooturl + '/huayan/ahlims.php?app=assay&act=index&tid='+asF.id+'&upfid='+$(this).val();
		...
		*/
		quxian_form_init : function(scF){
			...
			scF.find(".blue_a").andSelf().find(".change_xm,.change_by").unbind("click").click(function(){
				get_bzry_box(scF.vid,function(B){
	...
	*/
	 ...
	 * 功能描述：查看曲线标液
	*/
	var view_sc_bd = function(app,act,id,options,selQxByBox){
		(typeof options != "string") && (options = '');
		$.ajax({
	...
	*/
	var assay_sign = function(f,assay_type,sign_type,load_options){
		...
		if( f.canModi && 'fx_qz' == sign_type ){
			var column_json = ( '' == f.json ) ? {} : $.parseJSON(f.json);
	...
	*/
	window.get_bzry_box = function(vid,callBack){
		...
		if($("#sc_get_bzry_box").length==0){
			$.get(trade_global.rooturl+'/huayan/ahlims.php?ajax=1&app=quxian&act=getBzryBox&vid='+vid,
				function(content){
					$("body").append(content);
					var sc_gb_box = $("#sc_get_bzry_box");
					if($.isNumeric(vid)){
