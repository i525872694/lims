    ...
    switch($r['status']){
        ...
        case 8:
			if($u['jcbg_sh'] || $u['jcbg_qf']|| $u['userid']=="admin"){
				$operation="<a href=\"#\" onclick=\"return $(this).qbox({title:'检测报告列表',src:'$rooturl/baogao/bg_site_list.php?ajax=1&cyd_id=$r[id]',w:900,h:500});\" class=\"btn btn-xs btn-primary\" >报告查看下载</a>&nbsp;&nbsp;<a href=\"modi_bg_message_list.php?cyd_id=$r[id]&cy_date=$r[cy_date]\" class=\"btn btn-xs btn-primary\">修改报告信息</a>";
			}
            break;
    }

	$lines.=temp("bg/bg_liebiao_list.html");
}
//获得任务类型
