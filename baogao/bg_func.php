...
function water_pingjia($water_type,$vid,$result,$intent=''){
	...
	if($intent!='无' && $intent!=='0' && !empty($jcbz_arr)){
...
function is_chaobiao($vid,$water_type=0,$jcxz,$result,$is_eglish=0) {
	...
	if($over == '1' && !empty($xz_arr[0])){
		$beishu	= ($result-$xz_arr[0])/$xz_arr[0];
		//超标倍数 数据大于等于0.1时，小数点后保留1位，小于0.1时小数点后保留2为
		if($beishu < '0.1'){
...
function choose_date_html($set_content,$type='月报'){
	switch ($type) {
		...
		case '周报':
			...
			$week2		= empty($set_content['week2'])?'7':$week2;
			...
			$choose_date_html	= "起始日期:<select name='choose_date[week_type]' field='result_set' ><option value='本周'>本周</option><option value='上周' {$selected_month}>上周</option></select>
		...
		case '月报':
			...
			$day2		= empty($set_content['day2'])?'31':$set_content['day2'];
			$month_type	= $set_content['month_type'];
			//天数
