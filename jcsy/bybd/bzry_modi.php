...
for($i=1;$i<=4;$i++){
    if(''==$_POST['tj'][$i]) break;
    $kbyl[$i] = $_POST['kbyl'][$i]?$_POST['kbyl'][$i]:0;
    //计算公式
    if(floatval($_POST['mol_m'])){
        $bd_nd[$i]=_round((floatval($_POST['tj'][$i])*1000)/($_POST['yl'][$i])/$_POST['mol_m'],4);
    }else{
        $bd_nd[$i]=_round((floatval($_POST['jzry_nd'])*floatval($_POST['tj'][$i]))/($_POST['yl'][$i]-$kbyl[$i]),4);
    }
    $tj[$i] = $_POST['tj'][$i];
    $yl[$i] = $_POST['yl'][$i];
    $sjyl[$i]=($yl[$i]||$kbyl[$i])?$yl[$i]-$kbyl[$i]:'';
    $sum1+=$bd_nd[$i];
    $dnd[1]=_round($sum1/($i),4);
...
$bzry_nongdu=_round(($dnd[1]+$dnd[2])/2,4);
...
$sql_arr[] = "`kbyl`='".json_encode($kbyl)."'";
$sql_arr[] = "`sjyl`='".json_encode($sjyl)."'";
