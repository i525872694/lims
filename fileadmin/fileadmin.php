...
//print_rr($files);
$i = $xuhao	= 0; //rowspan=$juan_row rowspan=$lb_row
foreach($files as $file_key => $file){
	...
	if($juan_row !=''){
		...
		$xuhao_rowspan  = '1';
        $xuhao_rowspan_list     = [];
        foreach($file['juan'] as $juan_list){
                $xuhao_rowspan_list[]   = count($juan_list['lb'])+1;
        }
        if(count($xuhao_rowspan_list) > 0){
                $xuhao_rowspan  = array_sum($xuhao_rowspan_list)+1;
        }
		$lines .= "<tr>
				<td rowspan='{$xuhao_rowspan}'>{$xuhao}</td>
				<td class='lei_class'>
					...
					<a href='#' class='red icon-remove bigger-140' title='删除' onclick='del_lx({$file['id']})' ></a>
				</td>
		...
		foreach($file['juan'] as $juan_key=> $juan){//循环本要素下的卷
			...
			$lines .="<tr>
						<td title='点击修改卷名' class='juan_class' name_data='{$file['namebak']}' rowspan=$lb_row><span style='cursor:pointer' onclick='up_juan(this,{$juan['id']})'>{$juan['name']}</span>
							...
							<a href='#' class='red icon-remove bigger-140' title='删除' onclick='del_juan({$juan['id']},{$juan['pid']})' ></a>
						 </td>
