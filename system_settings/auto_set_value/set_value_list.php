...
//ajax添加模板名称
if($_GET['add_name']){
	$query_mb=$DB->query("SELECT * FROM n_set WHERE module_value2='".$_GET['add_name']."' AND `fzx_id`='{$fzx_id}'");
	if(!mysql_num_rows($query_mb)){
		$DB->query("INSERT INTO n_set(fzx_id,module_name,module_value2)values('{$fzx_id}','xm_px','".$_GET['add_name']."')");
		if(mysql_affected_rows()){
...
//查询出所有的模板
$option_px_mb='';
$n_set_sql="SELECT * FROM n_set WHERE module_name='xm_px' AND `fzx_id`='{$fzx_id}'";
...
//查出模板信息
$n_set	=	"SELECT * FROM `n_set` WHERE module_name='xm_px' AND `fzx_id`='{$fzx_id}' $px_id_str";
