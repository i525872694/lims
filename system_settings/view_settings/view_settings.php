...
//选项为A-Z
$str_option_arr = range(A,Z);
array_unshift($str_option_arr,' ');
...
foreach($global['site_type'] as $key=>$value){
	$str_option = disp_options($str_option_arr,0,$moren_site_type_mark[$key]);
...
while ($rs_water_type	= $DB->fetch_assoc($water_type_sql)) {
	...
	$str_option = disp_options($str_option_arr,0,$rs_water_type['bar_code_mark']);
	$bar_code_water_type .= "<label>{$rs_water_type['lname']}<select parent='{$rs_water_type['id']}' old_value='' name='water_type_mark[{$rs_water_type['id']}]'>$str_option</select></label>";
	...
	while ($rs_water_type2 = $DB->fetch_assoc($water_type_sql2)) {
		...
		$str_option = disp_options($str_option_arr,0,$rs_water_type2['bar_code_mark']);
		$bar_code_water_type	.= "<label>{$rs_water_type2['lname']}<select parent_id='{$rs_water_type['id']}' old_value='' name='water_type_mark[{$rs_water_type2['id']}]'>$str_option</select></label>";
...
disp("system_settings/view_settings.html");
?>
