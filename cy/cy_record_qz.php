...
//查询采样单中现有的签字日期,并按配置信息生成最新的签字日期
$sql = "SELECT `cy_rwjs_qz_date`,`cy_user_qz_date`,`sh_user_qz_date` FROM `cy` WHERE id='".$_GET['cyd_id']."'";
$query = $DB->query($sql);
$cyddate = $DB->fetch_assoc($query);
foreach ($cyddate as $k => $v) {
	if($clear_sign_date){
		$$k = date('Y-m-d H:i:s');
	}else{
		if(isset($v) && $v[0]!=0){
	...
	*/
	...
	if(1==$rs_cy['status']){
		if($u['userid']==$rs_cy['cy_user']&&!empty($rs_cy['cy_user'])){
			...
			$DB->query("update `cy` set `cy_rwjs_user`='".$u['userid']."', cy_rwjs_qz_date = '".$cy_rwjs_qz_date."', status='2',`json`='{$cy_json}' where id='".$_GET['cyd_id']."'");
		}
		if($u['userid']==$rs_cy['cy_user2']&&!empty($rs_cy['cy_user2'])){
			$cy_json_arr= json_decode($rs_cy['json'],true);
			$cy_json_arr['userid_img']['cy_rwjs_user2']	= $u['userid_img'];//电子签名信息
			$cy_json	= JSON($cy_json_arr);
			$DB->query("update `cy` set `cy_rwjs_user2`='".$u['userid']."', cy_rwjs_qz_date = '".$cy_rwjs_qz_date."', status='2',`json`='{$cy_json}' where id='".$_GET['cyd_id']."'");
		}
	}
	//更新cy表
	//通过id查询出cy这条数据
	...
	if(($u['userid']==$rs_cy['cy_user']&&!empty($rs_cy['cy_user'])) || $u['admin']){
		...
		$cy_json	= JSON($cy_json_arr);
		$DB->query("update `cy` set `cy_user_qz`='".$u['userid']."',cy_user_qz_date = '".$cy_user_qz_date."',status='".$status."',`json`='{$cy_json}' where `id`='".$_GET['cyd_id']."'");
		$assay_pay_user .= " ,`sign_01`='{$u['userid']}',`sign_date_01`='".$cy_user_qz_date."'";
	}
	if(($u['userid']==$rs_cy['cy_user2'] || $u['admin'])&&!empty($rs_cy['cy_user2'])){
		$cy_json_arr= json_decode($cyd['json'],true);
		$cy_json_arr['userid_img']['cy_user_qz2']	= $u['userid_img'];//电子签名信息
		$cy_json	= JSON($cy_json_arr);
		$DB->query("update `cy` set `cy_user_qz2`='".$u['userid']."',cy_user_qz_date = '".$cy_user_qz_date."',status='".$status."',`json`='{$cy_json}' where `id`='".$_GET['cyd_id']."'");
		$assay_pay_user .= " ,`sign_012`='{$u['userid']}',`sign_date_012`='".$cy_user_qz_date."'";
	}
...
if($_GET['action']=='ypjs_user_qz'){
	...
	$cy_json	= JSON($cy_json_arr);
    $DB->query("UPDATE cy SET sh_user_qz = '".$u['userid']."',status='".$status."', sh_user_qz_date = '".$sh_user_qz_date."',`json`='{$cy_json}' WHERE id = '".$_GET['cyd_id']."'");
...
//gotourl($url[$_u_][1]);
gotourl("$rooturl/cy/cy_record.php?cyd_id=$_GET[cyd_id]&&cyd_bh=$_GET[cyd_bh]");
?>
