...
//导航
$trade_global['daohang'][] = array('icon'=>'','html'=>'采样验收记录表  '."<span style='font-size:20px;color:red;font-weight:bold'>"."(采样单编号:".$_GET['cyd_bh'].")"."</span>",'href'=>"$rooturl/cy/cy_ys.php?cyd_id={$_GET['cyd_id']}");
...
while( $row = $DB->fetch_assoc( $res ) )
{
    ...
    if($row['ys_zt']){
        ...
        $ypzt = "<textarea cols='10' name='ys_zt[$row[id]' rows='4'>$row[ys_zt]</textarea>";
    }else{
        if($_GET['print']){
            $ypzt = $row['ys_zt'];
        }else{
            $ypzt = "<textarea cols='10' name='ys_zt[$row[id]' rows='4'></textarea>";
        }
    }
    ...
    <tr>
        ...
        <td style=\"width:14%;\" >采样地点</td>
        <td style=\"width:5%;\" >样品类型</td>
        <td style=\"width:5%;\" >样品编码</td>
        <td style=\"width:15%;\" >样品状态</td>ht
        <td style=\"width:8%;\" >验收结论</td>
    </tr><tr>";
        } 
    }
    $i++;
