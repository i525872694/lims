...
foreach ($bh_arr as $key => $value) {
    ...
    if($_POST['mark'] == 'site_code' && $_POST['site_code'][$i]!='没有站码'){
        $dayin_code = $_POST['site_code'][$i];//站点编码
    }else{
        $dayin_code = $key;//样品编号
    }
	$dayin .= $dayin_header.'B381,137,2,9,2,4,85,N,"'.$dayin_code.'"
A383,46,2,2,2,2,N,"'.$dayin_code.'"
P'.$value.'
<xpml></page></xpml><xpml><end/></xpml>';
}
