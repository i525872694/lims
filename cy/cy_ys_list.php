...
foreach($result as $key=>$data)
{
	$cy_ys_str='';
	if($data['status']>='4'){
		$cy_ys_str="<a href='cy_ys.php?cyd_id={$data['id']}&&cyd_bh={$data['cyd_bh']}'>采样验收记录表</a>";
	}
    if($data['status']>'4'){
        $cy_ys_str="<a href='cy_ys.php?cyd_id={$data['id']}&&cyd_bh={$data['cyd_bh']}'>采样验收记录表</a>|<a href='print_ypdj_taizhang.php?cyd_id={$data['id']}' target='view_window'>打印样品登记台账</a>";
    }
	$i = $key+1;
	...
	$lines.=temp("cy/ys_list_line.html");
}
//获得任务类型
...
$all_href = "`./print_ypdj_taizhang.php?cyd_id=all&cy_date=$_GET[cy_date]`";
disp("cy/ys_list.html");
