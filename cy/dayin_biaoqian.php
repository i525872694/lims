...
$rec_query=$DB->query("SELECT cr.*,s.site_code FROM `cy_rec` as cr LEFT JOIN `sites` AS s ON cr.sid=s.id WHERE cr.cyd_id='".$_GET['cyd_id']."' AND cr.`sid`>='0'  ORDER BY id");//ORDER BY bar_code");// AND status='1' AND sid>-1 ORDER BY bar_code");
$i=0;
...
$bar_num_list	= [];
for ($i=0; $i <=20 ; $i++) { 
	$bar_num_list[]	= "<option value='$i'>$i</option>";
}
$i = 0;
$bar_num_str	= implode('', $bar_num_list);
foreach($data as $key=>$value){
	...
	if($value['sid']>0&&$value['zk_flag']<0){
		$value['site_name']=$value['site_name'];
	}
	//生成化验单前可以修改样品编号
	//$modify_bar_code	= ' onclick="alert(\'该批次已生成化验单，无法修改样品编号！\');" ';
	$modify_bar_code	= ' onclick="alert(\'采样系统已同步编号，禁止修改样品编号！\');" ';
    ...
    if(empty($value['site_code'])){
        $value['site_code'] = "没有站码";
    }
	if($value['status']=='-1'){
		$site_code_lines.='<tr tongji="tong"><td>'.$i.'</td><td>'.$value['site_name'].'</td><td '.$modify_bar_code.'>'.$value['bar_code'].'</td><td>'.$value['site_code'].'</td><td>未采到水样</td><td><font color="red"></font></td></tr>';
	}else{
		$site_code_lines.='<tr tongji="tong"><td>'.$i.'</td><td>'.$value['site_name'].'</td><td '.$modify_bar_code.'>'.$value['bar_code'].'</td><td>'.$value['site_code'].'</td><td>
    <input type="hidden" name="site_code[]" value="'.$value['site_code'].'" />
...
if($yp_bar){
	...
	<input type=\"submit\" value=\"打印样品编号\" class=\"btn btn-primary btn-sm no-print\" clicked='' mark='bar_code' />
    <input type=\"submit\" value=\"打印站点编码\" class=\"btn btn-primary btn-sm no-print\" clicked='' mark='site_code' />
			<input type=\"button\" value=\"打印站点编号对照表\" class=\"btn btn-primary btn-sm no-print\" onclick=\"var url = window.location.href;url+='&print=print';window.open(url);\" />";
}else{
	$anniu = "<input type=\"button\" value=\"点击编号\" class=\"btn btn-primary btn-sm no-print\" clicked='' onclick='bianhao(this)' cyd_id={$_GET['cyd_id']}>";
}
disp("cy/dayin_biaoqian.html");
?>
