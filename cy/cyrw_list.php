...
while( $row = $DB->fetch_assoc($res) )
{
	...
	if($u['admin']||$u['system_admin']||( ($u['xd_cy_rw'] || $u['xd_csrw'])&&$row['status']<='2' )){
		$row["delete"] = "|<a class='red icon-remove bigger-130' title=\"删除\" href='javascript:if(confirm(\"一旦删除，无法恢复，你确实要删除吗？\")){del_cy({$row[id]});} '></a>";
	}
...
foreach($result as $key=>$data)
{
	$cy_record_str='';
	if($data['status']>='1'){
		$cy_record_str="|<a href='cy_record.php?cyd_id={$data['id']}&&cyd_bh={$data['cyd_bh']}'>采样记录表</a>";
	}
	$modi_csrw_tzd_str='';
	if($data['status']<'5'){
		//样品接收人签字时候就不再显示修改及确认采样任务
		$modi_csrw_tzd_str = "|<a href='modi_csrw_tzd.php?cyd_id={$data['id']}&&cyd_bh={$data['cyd_bh']}' title='修改及确认采样任务'>修改</a>";
	}
	$i = $key+1;
	...
	//搜索文本
	$sql = "SELECT 
		GROUP_CONCAT(`bar_code` SEPARATOR ',') as `bar_codes`, 
		GROUP_CONCAT(`site_name` SEPARATOR ',') as `site_names` 
		FROM `cy_rec` WHERE `cyd_id`='$data[id]'";
	$rec_infos = $DB->fetch_one_assoc($sql);
	$search_arr = array(
		$cy_users,$cyd_bh,$data['group_name'],$data['cy_date'],$rec_infos['bar_codes'],$rec_infos['site_names']
	);
	$search_data = implode(',',$search_arr);
	$lines.=temp("cy/cyrw_list_line.html");
}
//获得任务类型
