...
$trade_global['daohang'] = array(
	...
	array('icon'=>'','html'=>'修改及确认采样任务 '."<span style='font-size:20px;color:red;font-weight:bold'>"."(采样单编号:".$_GET['cyd_bh'].")"."</span>",'href'=>'./cy/modi_csrw_tzd.php?cyd_id='.$_GET['cyd_id'])
);
if($u['userid'] == ''){
	nologin();
}
...
while($row = $DB->fetch_assoc($rec_query)) {
	...
	if(empty($row['water_type'])){
		$tmp_water_type	= explode(',', $cyd['water_type']);
		$water_type_list	= [];
		foreach($tmp_water_type AS $value_type){
			$water_type_list[]	= $value_type;
			$water_type_max	= get_water_type_max($value_type,$fzx_id);
			$water_type_list[]	= $water_type_max;
		}
		$water_type_str=" in (".implode(',', array_filter(array_unique($water_type_list))).")";
	}else{
		$water_type_max=get_water_type_max($row['water_type'],$fzx_id);
		if($row['water_type']==$water_type_max || empty($water_type_max)){
			$water_type_str='= '.$row['water_type'];
		}else{
