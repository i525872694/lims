...
//导航
$trade_global['daohang'][] = array('icon'=>'','html'=>'采样通知单 '."<span style='font-size:20px;color:red;font-weight:bold'>"."(采样单编号:".$_GET['cyd_bh'].")"."</span>",'href'=>'./cy/cy_tzd.php?cyd_id='.$_GET['cyd_id']);
$_SESSION['cy_tzd']	= $trade_global['daohang'];
//点击打印时的显示
if($_GET['print']){
	$bar='';
	...
	$update_button = "<a href='#' class='btn btn-primary btn-sm' onclick='update_cy()'><i class='icon-download bigger-130'></i>发送最新采样瓶数据</a> ";
...
if($_GET['action']=='update'){
	...
	$rec_arr  = [];
	$cyd_id = $_GET['cyd_id'];
	$old_cy_sql = "SELECT cr.*,cy.cy_date,cy.cy_user,cy.cy_user2,cy.xc_exam_value,cy.xc_huanjing FROM `cy_rec` AS cr INNER JOIN `cy` ON cr.cyd_id=cy.id WHERE 1 AND `cr`.`cyd_id` in ({$cyd_id}) AND `cy_user` NOT IN ('','委托方')";
...
get_int($_GET['cyd_id']);
//查询采样时间、任务类型、备注
$rs_cy= $DB->fetch_one_assoc("SELECT id,`site_type`,`cy_date`,`note`,`xc_exam_value`,`cyd_bh`,`group_name` FROM `cy` WHERE id ='".$_GET['cyd_id']."'");
$xc_exam_value	= explode(',', $rs_cy['xc_exam_value']);
$note = $rs_cy['note'];
...
$value_arr	= array_diff($value_arr, $xc_exam_value);
...
while( $row = $DB->fetch_assoc( $res ) )
{
	...
	$rowarr=array_diff(explode(',',$row[assay_values]),$xc_exam_value);
	  //获取json转换为数组
	...
	foreach($rq_data as $k=>$v)
	{
		...
		elseif($json_arr['rq'][$k]==0){ //empty会过滤掉为0的数据 
			$rq_num = 0;
		}else{
