...
$can_click_vid	= array();
//导航
//$trade_global['daohang'][] = array('icon'=>'','html'=>'个人任务','href'=>'main.php');
//$_SESSION['daohang']['main']    = $trade_global['daohang'];
//只检索最近两个月内数据
...
while( $row = $DB->fetch_assoc($res)){
    ...
    $m['js_cyrw'] = $row['cy_flag']
        ? '<a href="'.$rooturl.'/cy/cy_tzd.php?cyd_id='.$row['id'].'&action=cy" target="main">接受任务</a>'
...
if($u['jh']){
    ...
    if(!empty($rs['v3'])){
        if(strpos($rs['v3'],"','")){
           $vids_v2_str=" in ('".$rs['v3']."')";
        }else{
           $vids_v2_str=" = '".$rs['v3']."'";
        }
        $sql_sh="SELECT id,td31,sign_01,sign_02,sign_date_01,sign_date_02,assay_element,status FROM standard_curve WHERE fzx_id='$fzx_id' AND sign_04='' AND sign_03!='' AND vid ".$vids_v2_str."";
        $query_sh=$DB->query($sql_sh);
        $qx_sh_line='';
        $qx_sh_sum=0;
        while($rs_sh=$DB->fetch_assoc($query_sh)){
            if($rs_sh['status'] == '曲线被退回'){
                $qx_sh_status   = "<font color='red'>{$rs_sh['status']}</font>";
            }else{
                $qx_sh_status   = $rs_sh['status'];
            }
            $qx_sh_sum++;
             $qx_sh_line.="<tr align=center>
                            <td>".$qx_sh_sum."</td>
                            <td><a href='$rooturl/huayan/ahlims.php?app=quxian&act=index&id=$rs_sh[id]'>".$rs_sh['assay_element']."</a></td>
                            <td>".$rs_sh['sign_01']."</td>
                            <td>".$rs_sh['sign_date_01']."</td>
                            <td>".$rs_sh['sign_02']."</td>
                            <td>".$rs_sh['sign_date_02']."</td>
                            <td>".$rs_sh['td31']."</td>
                            <td>".$u['userid']."</td>
                            <td>$qx_sh_status</td>
                          </tr>";
...
//审核 任务 查询的sql 语句
$fusql  = "SELECT SUM(a.`over`='已复核') AS finish_over,SUM(a.`over`='已校核') AS other_over,SUM(a.`json` LIKE '%退回%' AND a.`over`='已复核') AS finish_tuihui ,SUM(a.`json` LIKE '%退回%') AS all_tuihui,MIN(c.`jcwc_date`) AS jcwc_date,MIN(a.id) AS id,a.vid,a.assay_element from assay_pay as a left join cy as c on a.cyd_id=c.id where a.fzx_id='$fzx_id' AND c.cy_date > '$last_date' AND a.`is_xcjc`='0' and a.over in ('已完成','已校核','已复核')  $tv3 GROUP BY a.`vid` ORDER BY MIN(c.`jcwc_date`),a.vid";
$shsj   = $DB->query($fusql);
$shsum  = 0;
$shsum_total = 0;//可以审核的任务总数
while( $rsh = $DB->fetch_assoc( $shsj ) ){
    $shsum++;
    $all_num     =  $rsh['finish_over'] + $rsh['other_over'];
    $shsum_total += $rsh['finish_over'];
    //突出显示未完成的进度栏
    $jindu_bianse= '';
    if($all_num != $rsh['finish_over']){
        $jindu_bianse   = " style='color:red;' ";
    }
    //突出显示被退回的单元格
    $tuihui_bianse  = '';
    ...
    if($rsh['all_tuihui'] > 0){
        $tuihui_bianse  = " style='color:red;' ";
        $tuihui_jindu   = "{$rsh['finish_tuihui']}/{$rsh['all_tuihui']}";
    }
    //判断是否有未完成的化验单
    if($rsh['other_over'] > 0){
        $no_finish_a    = "<a href='$rooturl/huayan/ahlims.php?app=pay_list&year=".date('Y')."&month=全部&uid=全部&vid={$rsh['vid']}&status=已校核' >查看未复核的化验单</a>";
    }else{
        $no_finish_a    = "<span style='color:#9DAEBF;' onclick=\"alert('化验单均已复核');\">查看未复核的化验单</span>";
    }
    if($rsh['finish_over'] > 0){
        $finsh_a    = " <a href='$rooturl/huayan/assay_form.php?qz=sh&tid={$rsh['id']}'>去审核化验单</a>";
    }else{
        $finsh_a    = " <span style='color:#9DAEBF;' onclick=\"alert('没有已复核的化验单');\">去审核化验单</span>";
    }
    $main_sh    .= "<tr>
        <td>{$shsum}</td>
        <td>{$rsh['assay_element']}</td>
        <td $jindu_bianse title='已复核的化验单/已校核的化验单'>{$rsh['finish_over']}/{$all_num}</td>
        <td>{$rsh['jcwc_date']}</td>
        <td$tuihui_bianse>$tuihui_jindu</td>
        <td>$no_finish_a  &nbsp;$finsh_a</td>
    </tr>";
}
$shsum  = $shsum+$qx_sh_sum;
...
if($u['yiqi_manage']=='1'){//登陆及权限判断
    $yqsql ="select * from `yiqi` where `fzx_id`='1' order by yq_sbbianhao asc,yq_mingcheng asc";//内部编号排序 
     $R=$DB->query($yqsql); 
       $mc=$a;
       $bgr=$d;   
      $i=$j= 1;
    ...
    while($r=$DB->fetch_assoc($R)){
      $operation= "<a href=\"javascript:if(confirm('你真的要删除么?\\n一经删除,无法恢复!')) 
        location='./app_modal/yiqi_list.php?del=1&id=$r[id]&url=main'\">删除</a> 
        |<a href='./app_modal/yiqi_update.php?id=$r[id]&url=main'>修改</a>";
...
*/      $color='';
        if(!empty($r['yq_jiandingriqi'])){
            $today=time();
            $r_time=strtotime($r['yq_jiandingriqi']);
            $date_day=gb_diffBetweenTwoDays($today,$r_time);
            if($date_day<=30){
                $color='red';
            }
        }
...
/**
 * 求两个日期之间相差的天数
 * @param string $second1
 * @param string $second2
 * @return number
 */
function gb_diffBetweenTwoDays ($second1, $second2)
{
  if ($second1 < $second2) {
    $tmp = $second2;
    $second2 = $second1;
