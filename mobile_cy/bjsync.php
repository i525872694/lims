...
if(!empty($savelist)){
	...
	foreach($savelist as $carr){
		...
		$cid    = $carr['id'];
		//找不到采样单 直接跳过
		$is_set=$DB->fetch_one_assoc("select `id` from `cy_rec` where `id`='$cid'");
		if(empty($is_set['id'])){ continue; }
		if($cid > 0 && !in_array($cid,$cy_qz_arr)){
			...
			foreach($carr as $key=>$value){
				...
				if($key=='xc_img'){
					//每次上传都会更新图片 目前无法判断图片是否存在 
					//将接收到的base64存储起来
					$DB->query("update `cy_rec` set `xc_img_base64`='$value' where `id`='$cid'");
					//处理js生成的json字符串 
					...
					//循环将base64生成图片
					$img_arr=array();
					foreach($value as $kkkk=>$img_base){
						$base_img = str_replace('data:image/jpeg;base64,','',$img_base);
						$type='.jpeg';
						// 生成的文件名与路径
						$img_path = date('Y-m-d-H-i-s').rand(1,1000).$type;
						$path='../cy/xc_img/';
						// 生成文件
						$image_url=$path.$img_path;
						file_put_contents($image_url, base64_decode($base_img), true);
						$img_arr[]=$image_url;
					}
					$value=json_encode($img_arr,JSON_UNESCAPED_UNICODE);
					//删除原有图片
					$info=$DB->fetch_one_assoc("select * from `cy_rec` where id='$cid'");
					$old_img_arr=json_decode($info['xc_img'],true);
					foreach($old_img_arr as $old_k=>$old_v){
						@unlink($old_v);
					}
				}
...
$csql   = "select cy_rec.*,cy.note as cynote,cy.cy_date as yqcydate, cy.group_name ,sites.jingdu as s_jd,sites.weidu as s_wd,sites.banjing,xc_img,leixing.parent_id  from cy inner join cy_rec on cy.id=cy_rec.cyd_id left join sites on sites.id=cy_rec.sid left join `leixing` on cy.water_type=leixing.id where cy.fzx_id='$fzx_id' and cy.`cy_date` >'$ulastym'  ORDER BY cy.`cy_date` desc, `cy_rec`.`id` ASC  LIMIT 800";//`cy_rec`.`modify_time` >'$ulastym' and `cy_rec`.`create_date`>'$ulastym'
$cR = $DB->query($csql);
while($row = $DB->fetch_assoc($cR)){
	...
	unset($row['json']);
	//如果有base64数据 返回发送到平板 直接解析
	if(!empty($row['xc_img_base64'])){
		$row['xc_img']=$row['xc_img_base64'];
	}
	$row    += $jsarr;
