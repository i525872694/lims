...
function init() {
  ...
  $("#zdlist li a").live(activate, function() {
    showcy(this);
  });
  ...
  $(".cy[data-t=1]").live('blur', function() {
    cysave(this)
  });
  $("#xc_img").die().live('change', function() {
    uploadBtnClick()
  });
...
function showcy(cyzd) {
  ...
  //如果当前水样类型找不到对应的采样字段 使用默认字段
 if(cy_record_bt[q_dqcy['water_type']]){
    var cy_record_bt_str=q_dqcy['water_type'];
 }else{
    var cy_record_bt_str='m';
 }
  if (q_dqcy['water_type'] && cy_record_bt[cy_record_bt_str]) {
    for (var cy_bt_one in cy_record_bt[cy_record_bt_str]) {
      if (cy_record_bt_content[cy_bt_one]) {
        //下拉菜单
        result += "<li>" + cy_record_bt[cy_record_bt_str][cy_bt_one] + ":<select name='" + cy_bt_one + "' class=cy data-t=1 >";
        ...
        if (cy_bt_one == 'cy_ms' || cy_bt_one == 'qi_wen' || cy_bt_one == 'water_height' || cy_bt_one == 'liu_l' || cy_bt_one == 'liu_s') { //此段代码后期应该改掉
          var type_leixing = 'number';
        } else {
          var type_leixing = 'text';
        }
        result += "<li>" + cy_record_bt[cy_record_bt_str][cy_bt_one] + ":<input type='" + type_leixing + "' class=cy data-t=1  name='" + cy_bt_one + "' value=''/></li>";
      }
    }
  }
  //现场检测项目输入框
  ...
  if (q_dqcy['xc_img'] != '') {
    var img_arr=JSON.parse(q_dqcy['xc_img']);
    var img_str='';
    for(i=0;i<img_arr.length;i++){
      img_str+='<img  class="upload_img" src="' + img_arr[i] + '" >';
    }
    $("#old_img").html(img_str);
    $('#xc_img_data').text(q_dqcy['xc_img']);
  } else {
    $("#old_img").html(' ');
    $('#xc_img_data').text('null');
  }
  $('#file_img').html("<div class='z_file'><input type='file'  id='xc_img' name='xc_img'></div>");
  // alert(result);
  ...
  if ($("input[name=cy_time]:eq(0)").val() == '') {
    ...
    $("input[name=cy_time]:eq(0)").val(hours + ':' + d.getMinutes() + ':' + d.getSeconds());
  }
  //});
  /*$(".cy").each(function(){
      if($(this).attr('data-t')==1){//说明是input 输入框
...
function cysave(v) {
  ...
  cy_shijian = $(".cy[name='cy_time']").val();
...
function processPoints() {
  ...
  navigator.geolocation.watchPosition(locFn, locFail, option);
}
...
function uploadBtnClick() {
  ...
  if (window.File && window.FileReader && window.FileList && window.Blob) {
    var filefield = document.getElementById('xc_img'),
      file = filefield.files[0];

    var compressValue = document.getElementById('xc_img_data');
...
function processfile(file, compressValue, uploadCompressFile, scope) {
  ...
  reader.onload = function(event) {
    ...
    image.onload = function() {
      ...
      uploadCompressFile.apply(scope,[resized]);
    }
  };
...
// 图片压缩处理完毕的操作
function uploadCompressFile(resized) {
  //json存储图片base64 如果已存在 则添加
  var pic_base=$('#xc_img_data').text();
  if(pic_base!=='null'){
    var pic_arr=JSON.parse(pic_base);
  }else{
    var pic_arr=[];
  }
  var pic_length=pic_arr.length;
  pic_arr[pic_length]=resized;
  var pic_json=JSON.stringify(pic_arr);
  $('#xc_img_data').html(pic_json);
  //添加新上传图片预览
  var old_img=$('#old_img').html();
  old_img=old_img+'<img class=upload_img src="' + resized + '">'
  $('#old_img').html(old_img);
  cysave('img');
}
//绑定图片点击删除事件
...
$(document).on('click','.upload_img',function(){
  ...
  var img_index2=img_index==0?1:img_index;
  var msg = "您真的确定要删除这张图片吗?"; 
  if (confirm(msg)==true){
    //从数据里删除 
    var pic_arr=JSON.parse($('#xc_img_data').text());
    pic_arr.splice(img_index,img_index2);
    var pic_json=JSON.stringify(pic_arr);
    $('#xc_img_data').html(pic_json);
    cysave('img');
    $(this).remove();//删除这个元素
