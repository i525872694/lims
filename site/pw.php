...
$trade_global['daohang'][]= array('icon'=>'','html'=>'站点导入','href'=>"{$rooturl}/site/pw.php");
$_SESSION['daohang']['pw'] = $trade_global['daohang'];
$fzx_id = $u['fzx_id'];
$flag   = $_POST['flag'];
...
//AND site_type='{$site_arr['site_type']}'
            ...
            if($charu){
            	if($update=='no'){
					...
					//发送数据到对接表中
					$curl_arr = array();
					$curl_arr['STCD'] = $site_arr['site_code'];
					$curl_arr['STNM'] = $site_arr['site_name'];
					$curl_arr['PRPNM'] = $site_arr['site_line'];
					$curl_arr['LYNM'] = $site_arr['site_vertical'];
					$curl_arr['STLC'] = $site_arr['site_address'];
					$curl_arr['FZX_ID'] = $fzx_id;
					$curl_arr['LIMS_ID'] = $site_new_id;
					$curl_data = array();
            	...
            	if($update  == 'no'){
	             ...
	             //    }
	                if(count($site_tjcs_arr)>0){
	                    ...
	                    $sql_insert_group   = $DB->query("INSERT INTO `site_group` SET `fzx_id`='1',`site_id`='$site_new_id',`group_name`='',`ctime`='".date('Y-m-d h:i:s')."',`assay_values`='$xm',`cuser`='{$u['userid']}',`act`='1',`site_type`='{$site_arr[site_type]}'");
	                }
