...
case 'snzk':
    ...
    foreach($vid_arr as $vid => $cyd_id_arr)
    {
        ...
        foreach ($cyd_id_arr as $key => $cyd_id) {
            ...
            //曲线
            $sql = "SELECT `td21`,`td18`,`td19`,`td20` FROM `standard_curve` WHERE  id='{$r['scid']}'";
            $quxian=$DB->fetch_one_assoc($sql);
            $qx=array();
            if($quxian!=''){
            	$qx['hgfc']='y='.$quxian['td20'].'x'.(($quxian['td19']>0)?'+'.$quxian['td19']:$quxian['td19']);
            ...
            if($r2a['count']){
                //加标的测定率
                $r2a['cdl']  = _round($r2a['count']/$r['c']*100,2);
                //范围
