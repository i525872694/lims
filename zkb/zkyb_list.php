...
$year_list=select_disp_option($year_arr,trim($_GET['year']));
...
function select_disp_option($arr,$selected){
    $tmp_str=$tmp_selected='';
    foreach($arr as $k=>$v){
        $tmp_selected=$v==$selected?"selected":'';
        $tmp_str.="<option name='{$v}' {$tmp_selected}>{$v}</option>";
    }
    return $tmp_str;
}
?>