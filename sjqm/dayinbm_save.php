...
$sql = "SELECT * FROM `sjqm` WHERE `sm_bh` != '' AND `id` = '{$_GET['id']}'";
$re = $DB->query($sql);
while($data = $DB->fetch_assoc($re)){
	if(empty($_GET['num'])){
		$bh_arr[$data['sm_bh']] = $data['kucun'];//要打印的样品编号
	}else{
		$bh_arr[$data['sm_bh']] = $num;//要打印的样品编号
	}
}
if($u[userid] == '') nologin();
$fzx_id	= $u['fzx_id'];
...
$stream = fopen("/tmp/dy_sjqm.txt", "w+"); //保留最后一次打印数据
fwrite($stream, $dayin);
