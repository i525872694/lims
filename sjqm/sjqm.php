...
//查找物品类型
$sql_select = "SELECT `type` FROM `sjqm` GROUP BY type ORDER BY id";
...
if($_POST['add']){
	if(!empty($_POST['name'])){
		...
		$sql="INSERT INTO `sjqm`(`fzx_id`, `name`, `nice_name`,`fenzi_shi`, `type`, `jibie`, `guige`, `kucun`,`KCtixing`,`danjia`, `danwei`, `gl_user`, `pihao`, `changjia`, `youxiaoqi`,`GQtixing`, `beizhu`, `add_user`, `add_date` , `sm_bh`) VALUES(
			'$fzx_id',
			'$_POST[name]',
			'$_POST[nice_name]',
			'$_POST[fenzi_shi]',
