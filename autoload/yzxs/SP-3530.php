...
for($i=0;$i<count($arr);$i++){
	...
	if(stristr($line,"MG/L")||stristr($line,"µG/ML")||stristr($line,"UG/ML")&&$dw_zt){
		$unit='MG/L';
		$dw_zt='';
	}
	...
	if($quzhi_zt=="start"&&(stristr($line,".")||$line=="0")){
		...
		if($cishu==3){
			...
			$zhi[$bar][$xmid]=$line;
			if($xmid=='154'){
				$zhi[$bar][153]=$line;//总铁和铁的标示一样
			}
			if($line!='0'){
				if(empty($unit_arr[$unit]['blws'])){
					$zhi[$bar][$xmid]=$line*$unit_arr[$unit]['hs'];
					if($xmid=='154'){
						$zhi[$bar][153]=$line*$unit_arr[$unit]['hs'];//总铁和铁的标示一样
					}	
				}else{
					$zhi[$bar][$xmid]=number_format(($line*$unit_arr[$unit]['hs']),$unit_arr[$unit]['blws']);
					if($xmid=='154'){
						$zhi[$bar][153]=number_format(($line*$unit_arr[$unit]['hs']),$unit_arr[$unit]['blws']);//总铁和铁的标示一样
					}
				}
			}
			$quzhi_zt = "stop";
