...
$zhi = $shujv = $xm_arr=$bar_code_arr=$jcxm_arr=array();
$quzhi_zt=$cishu='';
for($i=0;$i<count($arr);$i++){
    ...
    foreach($temp_line as $key=>$value){
        ...
        if($quzhi_zt=='start'){
            ...
            if($cishu==count($xm_arr)+1){
                $xishi = $value;
                foreach ($shujv as $kk => $vv) {
                    $zhi['vd27'][$bar][$xm_arr[$kk]]=number_format($vv/$xishi, 4);
                    $zhi['vd2'][$bar][$xm_arr[$kk]]=$xishi;
                }
                $shujv = [];
                $cishu=0;
                $quzhi_zt='stop';
