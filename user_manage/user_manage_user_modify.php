...
########取出所有权限
$checkboxQx = '';
$no_modify	= " onclick=\"return false;\" ";
foreach($qx_one_arr as $key=>$value){
        //判断是不是总中心，不然不显示 分中心管理权限！！！!!!未完成
        if(stristr($key,"group")){//权限按功能分组
    		$checkboxQx .= "<label style='display:block;background-color:#F5F5F5;font-weight:bold;' {$no_modify} >".$value."</label>";//<input type='checkbox' name='fenzu' valur='' />".$key."</label>";
    	}
        else{
		...
		if($rs_users[$key]=='1'){
			...
			$checkboxQx	.= "<label class='bianse' style='margin-bottom:1px;margin-left:1px;height:43px;line-height:43px;width:130px;border:0px #D7D7D7 solid;font-weight:bold;color:#008000;disp'><input type='checkbox' class=' $no_modify' groupid$group_id='yes'group='$group_str' name='qx[]' value='".$key."' checked  {$no_modify}  onclick=\"return false;\" />".$value."</label>";
		}else{
			...
			$checkboxQx	.= "<label class='bianse' style='margin-bottom:1px;margin-left:1px;height:43px;line-height:43px;width:130px;border:0px #D7D7D7 solid;'><input type='checkbox' groupid$group_id='yes'group='$group_str' name='qx[]' value='".$key."' {$no_modify} style='display:none;'  onclick=\"return false;\" />".$value."</label>";
		}
	}
}

disp("user_manager/user_manage_user_modify.html");
?>
