...
if(empty($_GET['fzx_id'])||$_GET['fzx_id']=='undefined'){
	$_GET['fzx_id'] = $fzx_id;
}

if($_GET['handle'] == 'find_fzx'){
	$fzx_id = $_GET['fzx_id'];
}
//如果是总中心，那么赋予查看分中心的权利
if($u['is_zz'] == '1'){
...
if(!$rsYear){
	$optionY .= "<option selected>".date('Y')."</option>";
}
//算出系统中这一年中已有的月份
...
while($rsHyd=$DB->fetch_assoc($queHyd)){
	$rsHyd['sign_date_01'] = date('Y-m-d',strtotime($rsHyd['sign_date_01']));
	$rsHyd['sign_date_02'] = date('Y-m-d',strtotime($rsHyd['sign_date_02']));
	if($_GET['count_type']=='huayan'){
...
foreach($userArr as $key=>$val){
	...
	if($val['yanchishu']!=0){
		...
		$hanshu = " onclick=\"window.location.href='qzHydList.php?fzx_id=$_GET[fzx_id]&user=$key&huayan_yanchi_tid={$val['yanchi_tid']}&ycJhTid=".$val['ycJhTid']."&ycFhTid=".$val['ycFhTid']."&year=".$year."&month=".$month."';\"";
		$classN = "class='tdbian'";
	}
	else $hanshu = $classN = '';
	...
	if($val['zongshu']!=0)$linesArr[$key]="<tr class='bian'><td>".$key."</td><td class='tdbian' title='点击查看".$key."这个月所有签字的化验单'  onclick=\"window.location.href='qzHydList.php?fzx_id=$_GET[fzx_id]&user=$key&huayan_tid={$val['all_tid']}&huayan_yanchi_tid={$val['yanchi_tid']}&jhTid=".$val['jiaohe']."&fhTid=".$val['fuhe']."&ycJhTid=".$val['ycJhTid']."&ycFhTid=".$val['ycFhTid']."&year=".$year."&month=".$month."';\">".$val['zongshu']."</td><td title='点击查看".$key."这个月延迟签字的化验单' ".$classN.$hanshu.">".$val['yanchishu']."</td><td>".$yanChiLv.$baifenbi."</td><td class='tdbian' onclick=\"newchart('".$key."','".$year."','".$qixian."','".$zhoumoZt."','".$_GET['count_type']."');\">年趋势图</td></tr>";
}
