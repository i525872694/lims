...
if(!$u['user_manage']){
	nologin("{$rooturl}/main.php");
}
//导航
$trade_global['daohang'][] = array('icon'=>'','html'=>'权限管理','href'=>'user_manage/user_manage_list.php');
$_SESSION['daohang']['user_manage_list']    = $trade_global['daohang'];
...
while($aGroup = $DB->fetch_assoc($group)){
    $i++;
    if($aGroup['px'] != $i){//分组统一按照1进行排序
        $DB->query("UPDATE `users` SET `px`='{$i}' WHERE `id`='{$aGroup['id']}'");
        $aGroup['px']   = $i;
    }
	...
	$lines.=temp("user_manager/user_manage_list_lines.html");
}
$line   = '';
if($no_group_id){
...
disp("user_manager/user_manage_list.html");
?>
