...
header("Content-Disposition: attachment; filename=复核，校核延迟签字统计表_".$_GET['fzx_name'].".xls");
if($_GET['handle'] == 'downLoad_list'){
	$fzx_id = $_GET['fzx_id'];
}
//如果是总中心，那么赋予查看分中心的权利
if($u['is_zz'] == '1'){
	...
	$re = $DB->query($sql);
	$fzx_select = "查看分中心：<select id='fzx_select' onchange='find_fzx();'>";
	while($data = $DB->fetch_assoc($re)){
		if($_GET['fzx_id'] == $data['id']){
			$fzx_select .= "<option value='{$fzx_id}' selected>{$data['hub_name']}</option>";
		}
...
$qixian_down = $_GET['qixian'];
if($_GET['zhoumoZt'] == 'yes'){
	$zm_down = "";
}else{
	$zm_down = "不";
}
...
//算出系统中所有的年份
$queYear = $DB->query("select distinct EXTRACT(year from create_date) as Cyear from `assay_pay` where fzx_id='$fzx_id' and over in ('已校核','已复核','已审核') order by create_date desc");
...
//算出系统中这一年中已有的月份
$queMonth = $DB->query("select distinct EXTRACT(month from create_date) as Cmonth from `assay_pay` where fzx_id='$fzx_id' and over in ('已校核','已复核','已审核') and year(create_date)='".$year."' order by create_date desc");
...
if($month=='all')$whereM = "";
else $whereM = "and month(create_date)='".$month."'";
//取出所有已经校核及以后状态的化验单
...
foreach($userArr as $key=>$val){
	...
	if($val['yanchishu']!=0){
		...
		$hanshu = " onclick=\"window.location.href='qzHydList.php?user=$key&ycJhTid=".$val['ycJhTid']."&ycFhTid=".$val['ycFhTid']."&year=".$year."&month=".$month."';\"";
		$classN = "class='tdbian'";
	}
	else $hanshu = $classN = '';
	...
	if($val['zongshu']!=0)$linesArr[$key]="<tr  style='text-align:center;'><td>".$key."</td><td class='tdbian' title='点击查看".$key."这个月所有签字的化验单'  onclick=\"window.location.href='qzHydList.php?user=$key&jhTid=".$val['jiaohe']."&fhTid=".$val['fuhe']."&ycJhTid=".$val['ycJhTid']."&ycFhTid=".$val['ycFhTid']."&year=".$year."&month=".$month."';\">".$val['zongshu']."</td><td title='点击查看".$key."这个月延迟签字的化验单' ".$classN.$hanshu.">".$val['yanchishu']."</td><td>".$yanChiLv.$baifenbi."</td></tr>";
}
@array_multisort($yanchilvArr,SORT_DESC,$linesArr);//根据延迟率排序
$lines = implode('',$linesArr);//将数组中的每一行结果转换成字符串(排序后)
disp('user_manager/qz_yanchi_1_download');
/* 
