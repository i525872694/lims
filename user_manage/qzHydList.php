...
$daohang = array(
		...
		array('icon'=>'','html'=>$daohang_html,'href'=>'user_manage/qzHydList.php?user='.$_GET['user'].'&jhTid='.$_GET['jhTid'].'&fhTid='.$_GET['fhTid'].'&ycJhTid='.$_GET['ycJhTid'].'&ycFhTid='.$_GET['ycFhTid'].'&year='.$_GET['year'].'&month='.$_GET['month']),
);
...
if(!empty($_GET['jhTid'])||!empty($_GET['fhTid'])){//总的此人签字的化验单
	/*if(!empty($_GET['jhTid'])&&!empty($_GET['fhTid']))$where = "a.id in (".$_GET['jhTid'].",".$_GET['fhTid'].")";
	elseif(!empty($_GET['jhTid'])&&empty($_GET['fhTid']))$where = "a.id in (".$_GET['jhTid'].")";
	elseif(empty($_GET['jhTid'])&&!empty($_GET['fhTid']))$where = "a.id in (".$_GET['fhTid'].")";
	if(!empty($_GET['jhTid']))$jhTid  = @explode(",",$_GET['jhTid']);
...
elseif((empty($_GET['jhTid'])&&empty($_GET['fhTid']))&&(!empty($_GET['ycJhTid'])||!empty($_GET['ycFhTid']))){//延迟此人签字的化验单
	$leibie = "延迟签字";
}
else{
	$leibie = "";
...
if($leibie!=""&&$where!=''){
	...
	while($rsHyd=$DB->fetch_assoc($queHyd)){
		...
		//print_rr($rsHyd);
		if($leibie=="所有"&&(@in_array($rsHyd['id'],$ycJhTid)||@in_array($rsHyd['id'],$ycFhTid)))$riRed = "style=\"color:red;\"";
		else $riRed = '';
