...
elseif($_POST['action']=='add'){//添加用户信息
	...
	if(!empty($_POST['user_name'])&&!empty($_POST['nickname'])){
		...
		$id=$DB->insert_id();
    		$DB->query("insert into `hn_users` set `uid`=$id");
		if($id>0){
			//发送数据到duijie表中
			$curl_arr = array();
...
elseif($_GET['action']=='delete'){//删除用户
	if(!empty($_GET['uid'])){
		...
		if($manage_user_num==1&&$user_qx['user_manage']=='1'){
			...
			$DB->query("delete from `hn_users` where uid='{$_GET['uid']}'");
			//发送数据到duijie表中
			$curl_arr = array();
			$curl_arr['FZX_ID'] = $fzx_id;
			$curl_arr['LIMS_ID'] = $_GET['uid'];
			$curl_data = array();
			$curl_data['data'] = $curl_arr;
			$curl_data['action'] = 'table';
