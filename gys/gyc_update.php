...
if(!empty($_FILES['fujian']['name'])){
	...
	foreach($_FILES['fujian']['name'] as $key=>$value){
		if(!empty($_FILES['fujian']['name'])){
			...
			$cnt     = count($xxx);
			$rand    = rand(0,100);
			$newname[] = date(ymdhis).$rand.".".$xxx[$cnt-1];
			$oldname[]=$_FILES['fujian']['name'][$key];
...
if(empty($_FILES['fujian']['name'][0])){
	...
	$fujian=$tmp_arr=array();
	foreach($newname as $k=>$v){
		$tmp_arr['oldname']=$oldname[$k];
		$tmp_arr['newname']=$v;
	}
	if($row['fujian']){
		$fujian=json_decode($row['fujian'],true);
	}
	$fujian[]=$tmp_arr;
	$fujian=json_encode($fujian,JSON_UNESCAPED_UNICODE);
}
//备用联系人和联系方式 处理
$lxr_arr=$_POST['lxr2'];
...
if($_POST['handle']=='add'){
	//现增加列表页供应商
	$sql_add = "INSERT INTO `gys_gl` (`fzx_id`,`sname` , `dz` , `gy_fwly` ,`lxr` , `lxdh` , `fujian` , `gid` , `dengji_name` , `first_dengji_time`,`lianxi`,`year_pingjia`) VALUES('$u[fzx_id]','$sname'  , '$dz','$gy_fwly','$lxr','$lxdh','$fujian','$gid','$dengji_name','$first_dengji_time','$lianxi','$year_pingjia')";
	if($DB->query($sql_add)){
		$id=$DB->insert_id();
