...
if(!empty($parent_id) || $_GET['handle']=='del_file'){
	...
	$id 	= $row['id'];
	$fujian_arr=json_decode($row["fujian"],true);
	if(empty($fujian_arr)){
		$fujian='';
	}else{
		...
		$fujian.=<<<ETF
				<ul>
					<li><a style="cursor:pointer;" data-toggle="modal" data-target="#myModal" onclick="show_img(this,$key);">$value[oldname]</a>&nbsp;&nbsp;&nbsp;&nbsp;<a style="color:red;cursor:pointer;" onclick="del_file($id,$key);">删除</a></li>
				 </ul>
				 <!--图片遮罩层-->
				  ...
				  <div class="modal-dialog" role="document">
				    <div class="modal-content">
				      <div class="modal-header">
				        ...
				        <h4 class="modal-title" id="myModalLabel">图片:$value[oldname]</h4>
				      </div>
				      <div class="modal-body">
				        <a href="$rooturl/gys/upfiles/$value[newname]" target="_blank;"><img src="$rooturl/gys/upfiles/$value[newname]" title="$value[oldname]"  style="width:50%;"/></a>
				      </div>
