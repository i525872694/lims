...
$sender_io->on('connection', function($socket){
    ...
    $socket->on('login', function ($uid)use($socket){
        ...
        // 更新对应uid的在线数据 存进数组里 更新user表数组状态
        $uid = (string)$uid;
    ...
    $socket->on('send', function ($uid)use($socket){
...
$sender_io->on('workerStart', function(){
        ...
        switch(@$_POST['type']){
            case 'publish':
                ...
                if($_POST['logout']&&$to){
                    ...
                    $sender_io->to($to)->emit('logout', 'logout');
                    exit;
                }
                //返回在线人数
                if($_POST['login_user']){
                    return $http_connection->send(json_encode($uidConnectionMap));
                    exit;
                }
    ...
    // 心跳，定时向所有uid推送当前uid在线数及在线页面 防止长时间未通讯被路由节点强行断开
