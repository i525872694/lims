...
class DbApp extends SiteRoot 
{
    ...
    function backup()
    {
        ...
        //echo "show tables LIKE '". $this->_mysql_like_quote(DB_PREFIX) . "%'";
        //print_rr($all_tables);die;
        $key = array_keys($all_tables,array('Tables_in_water (%)'=>'hy_shuyuan'));
        unset($all_tables[$key[0]]);    //hy_shuyuan表不备份
        $tables = array(); //备份队列
        ...
        if (!$_POST)
        {
            ...
            Config::assign('vol_size', array($allow_max_size,$allow_max_size/1024));
            Config::assign('backup_name', $this->_make_backup_name());
            Config::assign('tables', $tables);
            Config::display('db.backup.html');
        }
        else
        {
            ...
            if(!$backup_name)
            {
                $this->show_warning('备份名字不能为空');
                return;
            }
            if (!preg_match("/^[\w]{" . $this->limit_backup_name . "}$/", $backup_name))
            {
                $limit_arr = explode(',', $this->limit_backup_name);
                $this->show_warning(sprintf('备份名字由%s到%s位数字、字母或下划线组成', $limit_arr[0], $limit_arr[1]));
                return;
            }
            $this->backup_name = $backup_name;
            if(is_dir($this->backup_path . $this->backup_name))
            {
                $this->show_warning('备份名称已存在，请换一个');
                return;
            }
            /*开始备份*/
            lims_mkdir(ROOT_PATH . '/data/'. $this->backup_dir .'/' . $this->backup_name);
            $this->new_export();
        }
    }

    function new_export(){
        global $db_user,$db_pass,$server,$dbname;
        exec(' mysqldump -u '.$db_user.' -p'.$db_pass.' '.$dbname.' | gzip >'.$this->backup_path.$this->backup_name.'/'.$this->backup_name.'.sql.gz',$out,$status);
        //exec(' mysqldump -u root -p63508790 water >'.$this->backup_path.trim($_POST['backup_name']).'/'.date('Y_m_d_His').'.sql ',$out,$status);
        /* 备份结束 */
        if ($status == 0)
        {
            ...
            Config::assign('list',  $list);
            Config::assign('title', '备份成功');
            Config::display('db.message.html');
        }else{
            $this->show_warning('备份写入失败');//这里需要处理
        }
    }
    /**
     *    数据表导出
    ...
    function export()
    {
        ...
        {
            $this->show_warning(sprintf('分卷大小至少为%sK', $this->min_vol_size));
            return;
        }
    ...
    function restore()
    {
        ...
        Config::assign('backups', $backups);
        Config::display('db.restore.html');
    }
    ...
    function drop()
    {
        ...
        {
            is_dir($this->backup_path.$backup_name) && lims_rmdir($this->backup_path.$backup_name);
        }
        $this->Reto(2, 'index.php?app=db&act=restore', '删除成功');
    ...
    function import()
    {
        ...
        {
            if ($backup['lims_ver'] != VERSION)
            {
                $this->show_warning(sprintf('LIMS 当前版本%s与备份数据版本%s不同，备份恢复失败', VERSION, $backup['lims_ver']));
                return;
            }
        }
        /* 检查是否完成 */
        ...
        {
            $this->show_warning('该备份未完成，可能是备份时被中止，不能继续导入');
            return;
        }
        /* 检查完整性 */
        $end_vol = end($backups);
        ...
        if ($total != count($backups) || !$total){//检查卷数量
            $this->show_warning('分卷不完整或分卷信息有错误，不能继续导入');
            return;
        }
        ...
        if (!$vol || !$backup)
        {
            $this->show_warning(sprintf('分卷%s不存在', $vol));
            return;
        }
        /*开始导入sql*/
        if ($this->_import_vol($backup['file']))
        {
            ...
            if ($vol<count($backups))
            {
                ...
                Config::assign('title', sprintf('分卷数据 %s 成功导入，程序将自动继续。', '#' . $vol));
                Config::assign('auto_redirect', 1);
                Config::assign('auto_link', $lnk);
                Config::display('db.message.html');
            }
            else
            {
                $this->_drop_finish_vol('im_finish_vol.log');
                $this->Reto(2, '', '导入成功',JSTurn(-1,2000));
                //$this->show_message('导入成功');
            }
        }
        else
        {
            $this->show_warning(sprintf('导入分卷%s失败', $backup['file']));
            return;
        }
    }
    ...
    function _deal_result($tables, $vol, $vol_size)
    {
        ...
        else
        {
            ...
            Config::assign('title', sprintf('分卷数据 %s 成功创建，程序将自动继续。', '#' . $vol));
            Config::assign('auto_redirect', 1);
            Config::assign('auto_link', $lnk);
            Config::display('db.message.html');
        }
    ...
    function _dump_queue($vol, $vol_size)
    {
        ...
        $this->dump_sql .= "-- END LIMS 2.0 SQL Dump Program ";
        if (empty($tables))
        {
    ...
    function _make_head($vol)
    {
        ...
        $sys_info['LIMS_version']     = VERSION . ' (' . CHARSET . ')';
        $sys_info['date']       = date('Y-m-d H:i:s');

        $head = "-- LIMS 2.0 SQL Dump Program\r\n" .
                 "-- " . $sys_info['web_server'] . "\r\n" .
                 "-- \r\n" .
                 "-- DATE : ".$sys_info["date"]. "\r\n" .
        ...
        if (is_dir($vol_path))
        {
            if ($handle = opendir($vol_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    ...
                    if ($file_info['extension'] == 'gz' || $file_info['extension'] == 'sql')
                    {
                        $vol = $this->_get_head($vol_path . $file);
                        $vol['file'] = $file;
                        $bytes += filesize($vol_path . $file);
                        $vol['size'] = round(ceil(10 * filesize($vol_path . $file) / 1024) / 10 /1024,2) .'M';
        ...
        if (is_dir($this->backup_path))
        {
            if ($handle = opendir($this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    ...
                    {
                        $backup['name'] = $file;
                        $backup['date'] = date('Y-m-d H:i:s',filemtime($this->backup_path . $file));
                        $backup['vols'] = $this->_list_vol($file);
                        $end_vol = end($backup['vols']);
                        $backup['total'] = isset($end_vol['total']) ? intval($end_vol['total']) : '';
                        $backup['total_size'] = isset($end_vol['total_size']) ? $end_vol['total_size'] : '';
    ...
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'lims_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 317);
        fclose($fp);
        $arr = explode("\n", $str);
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'LIMS VERSION')
                {
                    $sql_info['lims_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
