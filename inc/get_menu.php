...
$sql_where	= '';
if(!empty($_GET['type'])){
	$sql_where	= " AND `type`='{$_GET['type']}' ";
}
$parent_qx	= array();
$p_query  = $DB->query("SELECT * FROM `menu` WHERE `parent_id` = '0' {$sql_where} ORDER BY `sort`");
while($parent = $DB->fetch_assoc($p_query)){
	...
	if(in_array($parent['name'],array('下达采样任务','监测计划下达')) && !empty($global['site_type'])){
		//判断有没有 下达采样任务的权限
		if(!empty($menu[$parent['id']])){
			foreach($global['site_type'] as $key=>$val){
				if(stristr($val,'新版下达任务')){
					$menu[$parent['id']]['c'][]	= array('id'=>'','parent_id'=>'','name'=>$val,'url'=>"xd_cyrw/xd_cyrw_index_select.php target=main",'sort'=>'','title'=>"下达".$val,'qx'=>'','icon'=>'');
				}else{
