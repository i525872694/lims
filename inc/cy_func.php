...
*/
function new_bar_code( $site_type,$water_type, $cy_date,$fix_bar_code='' ) {
	global $global,$DB,$u,$cy_rec_info,$old_bar_code,$qckb_zk;
	$fzx_id = $u['fzx_id'];
	$bar_code	 = $where_water_type	= '';
	if(empty($fix_bar_code)){
		...
		if(!empty($bar_code_make_old['module_value1'])){
		    ...
		    //默认全部包括
		    $bar_code_make_arr  = array("site_type","water_type","year","month","-","serial_num",'4');
		}
		foreach ($bar_code_make_arr as $value) {
			switch ($value) {
				case 'fzx_mark':
					$bar_fzx_mark_arr   = $DB->fetch_one_assoc("SELECT * FROM `hub_info` WHERE `id`='{$fzx_id}'");
		    		$bar_fzx_mark       = $bar_fzx_mark_arr['bar_code'];
					break;
				case 'site_type':
					#########取出样品编号中任务类型所代表的字母
				...
				case 'month':
					...
					break;
				case 'serial_num':
					$liushuihao_num	= end($bar_code_make_arr);
					break;
				case '-':case 'fzx-':
					# code...
					break;
		...
		$fix_bar_code	= trim($fix_bar_code);
		if($cy_rec_info['zk_flag'] =='-6'){//现场平行样的编号
			$bar_code	= $old_bar_code."X";
			return $bar_code;
		}else if($qckb_zk =='1'){//全程序空白样
			if(!strstr($old_bar_code,'K')){
				$bar_code       = $old_bar_code."K";
			}else{
				$bar_code       = $old_bar_code;
			}
			return $bar_code;
		}else{
			if(preg_match('/^[A-Z]{2}\d+/', $fix_bar_code)){
				$bar_code			= substr($fix_bar_code,0,2);
				$bar_code_make_arr	= array("site_type","water_type","year","month","-","serial_num",'4');
			}
		}
	}
	...
	foreach ($bar_code_make_arr as $value) {
		switch ($value) {
			case 'fzx_mark':
				if(empty($bar_fzx_mark)){
					$bar_code	.= '*';//没有配置就默认为*
				}else{
					$bar_code	.= $bar_fzx_mark;
		 		}
				break;
			case 'site_type':
				//根据任务类型确定 样品编号的第一位
			...
			case 'water_type':
				...
				break;
			case 'year':
				$bar_code	.= date('Y', strtotime($cy_date));
				break;
			case 'month':
				$bar_code	.= date('m', strtotime($cy_date));
				break;
			case 'serial_num':
				...
				if(empty($fix_bar_code)){
					$sql_liushuihao_num	= $liushuihao_num + 1;
					$last_bar_sql	= "SELECT cy.site_type,rec.id,rec.bar_code FROM `cy_rec` AS rec INNER JOIN `cy` ON rec.cyd_id=cy.id WHERE cy.fzx_id='$fzx_id' AND cy.site_type='$site_type' $where_water_type AND rec.bar_code!='' AND cy.cy_date>='$where_date' $where_month AND `bar_code` like '{$bar_code}%' AND (SUBSTR(bar_code,-1)*1 > 0 || SUBSTR(bar_code,-1) ='0') ORDER BY SUBSTR(bar_code,-{$liushuihao_num})*1 DESC  LIMIT 0,1";
					$rs_bar_code	 = $DB->fetch_one_assoc($last_bar_sql);
					//样品编号流水号部分
					if(!empty($rs_bar_code['bar_code'])){
						$new_num	= (int)substr( $rs_bar_code['bar_code'], -$liushuihao_num) + 1;
					}else{
						$new_num	= 1;
					}
					$now_liushuihao_num	= strlen($new_num);
					$t	= '';
					for ($i=$now_liushuihao_num; $i < $liushuihao_num; $i++) {
						$t	.= '0';
					}$bar_code	.= $t . $new_num;
				}else{
					$bar_code	.= substr($fix_bar_code,-4);
				}
				break;
			case '-':case 'fzx-':
				$bar_code	.= '-';
...
function get_water_type_max($water_type,$fzx_id){
	global $DB;
	$lx=$DB->fetch_one_assoc("SELECT id,parent_id FROM	`leixing` WHERE id='".$water_type."' AND (fzx_id='".$fzx_id."'||fzx_id='0'||fzx_id='1')");
	if(!$lx){
		return false;
	}else{
		if($lx['parent_id']=='0'){
