...
#################站点管理，分中心站搜索批次
function get_group_names( $site_type_num = 0, $water_type = '' ) {
    global $DB;global $u;
	if($water_type!=''&&$water_type!='全部'){
		$water_type_find=" and s.water_type='".$water_type."'";
	}
    if($site_type_num!=''&&$site_type_num!='全部'){
        $site_type_find=" and s.site_type='".$site_type_num."'";
    }
    $result = array();
	if($u['xiangxi']!=1){
		$act="and sg.sort<10000";
	}
	$sql="SELECT DISTINCT sg.group_name FROM `site_group` sg  LEFT JOIN sites s ON s.id = sg.site_id  WHERE sg.fzx_id='".$u['fzx_id']."' ".$site_type_find." ".$water_type_find."  ".$act."  AND sg.group_name!='' AND sg.act='1' ORDER BY sg.group_name";
