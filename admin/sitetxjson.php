...
$sql	= "SELECT ao.vd0,ao.id,cr.cy_date,ao.site_name  FROM  `assay_order` as ao LEFT JOIN cy_rec as cr ON ao.cid=cr.id LEFT JOIN `sites`  AS s ON cr.sid=s.id WHERE ao.sid =  '$sid' AND ao.vid ='$vid' and ao.hy_flag>=0 $where_sql ORDER BY  `cr`.`cy_date` ASC ";
$R		= $DB->query($sql);
