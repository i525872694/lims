...
if(!empty($cyd_id_arr)){
	...
	$hyd_sql="select a.cyd_id,ap.userid, a.create_date,SUM(a.`assay_over`='1' or a.`assay_over`='over') as finish_value,count(a.`id`) as all_value,SUM(ap.`is_xcjc`='1') as xc_value,ap.fzx_id from assay_order a join assay_pay ap on ap.id=a.tid where a.cyd_id in ($cyd_id) group by a.cyd_id  ORDER BY ap.fzx_id";
	...
	while($hyd_sel=$DB->fetch_assoc($hyd_query)){
		...
		if($hyd_sel['all_value'] != $hyd_sel['xc_value']){
			...
			if($old_fzx!= $hyd_sel['fzx_id']){
				...
				if(empty($fzx_arr[$hyd_sel['fzx_id']])){
					if(empty($fzx_arr)){
						...
						$fzx_arr[$hyd_sel['fzx_id']]    = "未找到该分中心名称（{$hyd_sel['fzx_id']}）";
					}
				}
			...
			$lines.="<tr>
					...
					<td><a href='$rooturl/huayan/ahlims.php?app=pay_list&cyd_id={$hyd_sel['cyd_id']}&year=$riqiy&month=$riqi&fzx={$hyd_sel['fzx_id']}&fx_user=全部'>查看检测任务</a>&nbsp;|&nbsp;<a href='$rooturl/cy/cy_record.php?cyd_id={$hyd_sel['cyd_id']}'>查看采样任务</a></td>
				</tr>";
		}
	}
}//是否有查询出数据的查询
