...
$sql    = "SELECT cy.id,cy_rec.bar_code,cy_rec.juli ,cy_rec.sid,cy_rec.jingdu,cy_rec.weidu , cy.cyd_bh,cy.cy_user,cy.group_name FROM cy,cy_rec WHERE year(cy.`cy_date`) = '$_GET[year]' AND month(cy.`cy_date`) = '$_GET[month]'  AND cy.id = cy_rec.cyd_id   and  zk_flag>=0  and  zk_flag!=1   $fzx_sql ";
...
$sql   .= "  order by cy.group_name , cy_rec.bar_code "; 
// echo $sql;
$res    = $DB->query( $sql );
