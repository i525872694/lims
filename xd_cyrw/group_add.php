...
if($_GET['action']=='group_add' || $_GET['action']=='group_modify'){
	...
	if($_GET['action']=='group_modify'){
		...
		while($rs_this_group = $DB->fetch_assoc($sql_this_group)){
			...
			$site_label	.= "<label title='{$rs_this_group['river_name']}.{$rs_this_group['site_name']}{{$_GET['group_name']}}' class='group_sites'><input type='checkbox' name='sites[]' site_id='{$rs_this_group['id']}' value='{$rs_this_group['id']}' checked />{$rs_this_group['site_name']}<font color='#9B9898'>$line_vertical</font></label>";
		}
	}else{
		$title  = "采样批次添加";
		$button_str	= "确认添加";
...
echo temp("xd_cyrw/group_add.html");
?>
