...
if(!empty($_POST)){
	...
	if(empty($_POST['sites']) && !empty($_POST['alone_sites'])){
		$_POST['sites'][]	= $_POST['alone_sites'];
	}
	...
	if(!empty($report_order_list)){
		foreach ($report_order_list as $key_cid => $vid_result) {
			foreach ($vid_result as $key_vid => $value_result) {
				$value_result['vd0']   = str_replace('＜', '<', $value_result['vd0']);//转换下小于号，防止用户手动输入的数据无法判断
				if(stristr($value_result['vd0'],'<')){
					$value_result['vd0']	= (str_replace('<','',$value_result['vd0']))/2;
				}
				$vd0		= "'".$value_result['vd0']."'";
				$site_name	= $report_rec_list[$key_cid]['site_name'];
				$cy_date	= $report_rec_list[$key_cid]['cy_date'];
				$sid		= $report_rec_list[$key_cid]['sid'];
				//同一站点的全部采样日期
				if(@!in_array($cy_date, $site_date_arr[$sid])){
					$site_date_arr[$sid][]	= $cy_date;
				}
				//同一项目的全部采样日期
				if(@!in_array($cy_date, $site_date_arr[$key_vid])){
					$site_date_arr[$key_vid][]	= $cy_date;
				}
				//post存储 site
				if(@!array_key_exists($sid,$result_site_arr)){
					$html_site	.= $site_name."、";
				}
				//post存储 vid
				if(@!array_key_exists($key_vid,$result_vid_arr)){
					$html_vid	.= $xm_name_list[$key_vid]."、";
				}
				//站点名称
				$site_name_arr[$sid]	= $site_name;
				//结果值
				$result_site_arr[$sid][$key_vid][$cy_date]	= $vd0;
				$result_vid_arr[$key_vid][$sid][$cy_date]	= $vd0;
			}
		}
	}else{
